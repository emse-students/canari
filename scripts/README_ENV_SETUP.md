...
This applies to the Bash script (`setup-env.sh`) only.
The PowerShell script (`setup-env.ps1`) uses .NET cryptography and does not require OpenSSL.

Install OpenSSL (for Bash script):
