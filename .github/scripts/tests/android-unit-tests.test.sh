...
# exists. A task name that matches nothing makes Gradle exit 0 having run nothing - measured on
# the old flavoured `app` module on 2026-09-10, `BUILD SUCCESSFUL in 16s`, zero tests, from the
# very command the backlog entry had proposed. A gate that green is the defect it was written to
# remove, and it is not specific to that layout: any renamed task reproduces it.
#
...
case "$mode" in
  ran)
    ...
    cat > build/test-results/test/TEST-fr.emse.canari.PushDecryptLadderTest.xml <<XML
...
XML
  ...
  zero)
    mkdir -p build/test-results/test
    cat > build/test-results/test/TEST-nothing.xml <<XML
<?xml version="1.0" encoding="UTF-8"?>
...
check "tells the reader how to find the real task name" empty 1 "tasks --all"
...
mkdir -p "$work/android/build/test-results/test"
printf '<testsuite name="stale" tests="9" failures="0"></testsuite>\n' \
  > "$work/android/build/test-results/test/TEST-stale.xml"
