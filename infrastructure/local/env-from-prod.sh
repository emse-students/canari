...
for name in $PROD_NAMES; do
  hits=0
  for list in "$KEEP" "$BLANK" "$REGENERATE" "$LOCAL" "$OIDC"; do
...
if [ -n "$UNKNOWN" ]; then
  ...
  fail "add each to KEEP, BLANK, REGENERATE, LOCAL or OIDC above - which one is a decision, not a default"
fi
...
#
# This file carries PRODUCTION credentials for every third party: Lydia, FCM, APNs, Cloudflare,
# MiGallery, Safe Browsing and the inter-app API keys. Decided by the user on 2026-09-02, so that
...
HEADER
  ...
  printf '\n# ── Generated for this machine on FIRST run, preserved afterwards (auth, not data) ──\n'
  # PRESERVED ON A RE-RUN, AND THAT IS NOT AN OPTIMISATION. A second run used to mint four new
  # secrets, which desynchronises this file from the CONTAINERS already started with the old ones -
  # every internal HMAC call then fails with a signature error that says nothing about a .env having
  # been rewritten. The values carry no production meaning, so keeping them costs nothing; what
  # would cost is a stack whose halves disagree. The previous file is read from the .bak this
  # script has just written.
  for name in $(printf '%s\n' "$REGENERATE" | tr ' ' '\n' | grep -v '^$' | sort); do
    previous=""
    [ -f "$INFRA_ENV.bak" ] && previous=$(sed -n "s/^$name=//p" "$INFRA_ENV.bak" | head -1)
    if [ -n "$previous" ]; then
      printf '%s=%s\n' "$name" "$previous"
      KEPT_SECRETS="$KEPT_SECRETS $name"
    else
      printf '%s=%s\n' "$name" "$(openssl rand -hex 32)"
      NEW_SECRETS="$NEW_SECRETS $name"
    fi
  done
...
[ -n "$NEW_SECRETS" ] && log "  auth secrets GENERATED:$NEW_SECRETS"
[ -n "$KEPT_SECRETS" ] && log "  auth secrets kept from the previous file:$KEPT_SECRETS"
# A generated secret is only in step with a RESTARTED stack: the containers read this file at start.
[ -n "$NEW_SECRETS" ] && [ -n "$KEPT_SECRETS" ] && log \
  "  NOTE: some were generated and some kept - restart the stack so every container agrees"
