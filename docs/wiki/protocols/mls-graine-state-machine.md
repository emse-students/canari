...
| D5 | Sweep local groups against the server list | `readGroupSweepSnapshot` + `forgetGroupsAbsentFromServer` (`groupSweep.ts`), called by the connection sync and by `discoverMissingGroups` | n/a since 2026-09-14 | **FUSED 2026-09-14, and it was FOUR divergences rather than one duplicate.** Only the sync refused an empty list, only discovery de-duplicated the rows, only the sync logged a failed fetch, and only the sync printed the reason a group was forgotten. `reconcileAbsentLocalGroup` now has exactly one caller and neither sweep fetches the list itself. P2-2 done. |
...
| DE5 | **An outbox entry held for ever** | `!isGroupHealthy` holds the entry, and the two permanent dispositions (`group-deleted`, `evicted`) do not cover a group nobody can repair | **NO, AND HALF OF IT IS FIXED (2026-09-14).** The hold had neither a clock nor a counter: it returned `'retry'` without writing `attempts` or `nextAttemptAt`, so the entry contributed nothing to `scheduleBackoff` and was retried only when something unrelated flushed. Both are written now, through the one `holdForRetry`. **What is still open is the terminal disposition**, and it is open on a QUESTION rather than on work: a group in `NO_REPAIRER` may become repairable the moment a holder comes online, so no count and no clock is a proof that it will not. P2-5. |
...
- ~~**P2-2. Collapse D5**, the two group sweeps, into one predicate with one implementation.~~
  **DONE 2026-09-14.** The row said "two copies of one predicate"; converting them found that the
  two had drifted apart on **four** axes, and the most important one was DESTRUCTIVE. The sync
  refuses to read absence off a list that came back EMPTY while the device holds trees - an account
  with live MLS state always has `dm_group_members` rows server-side, and leaving or deleting a
  group drops the local tree in the same breath, so "empty list, local trees" is a transient answer
  rather than a state a user can reach. **Discovery had no such guard**, and its own per-group
  answer is the destructive branch: `dm_groups row alive, naming no distribution scope, absent from
  our group list` - a reason citing a precondition the caller had not established. A single empty
  200 would have forgotten every tree the device held. `initializeConnection` asserted in a comment
  that its twin carried the same guard; it did not, and that sentence is how the divergence lasted.
  The other three: only discovery de-duplicated the server rows (and its comment said "keep first
  occurrence" while `new Map` kept the LAST), only the sync logged a failed fetch (`catch {}` on
  the other side), and only the sync printed the `reason` with the removal - the one sentence a
  reader reaches for when a group is gone and nobody knows why.
