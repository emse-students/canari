...
**Every bundle names the device it answers.** A `history_bundle` is a group frame, so all members receive an answer meant for one of them. `bundleFrame` (`groupActions.ts`) puts the requester's `digestIdentity` in `to` on every send path - the full store, the id-filtered diff, and both flavours of empty bundle - and the receiver splits what it does with the frame in two: the MESSAGES are ingested by everyone (deduped by id, so over-delivery costs bandwidth), the ANSWER (`noteHistoryBundleReceived`, which discharges the durable marker) is read only by the addressee. Before this, one repair between two peers cleared the marker of every other member - permanently, since the marker is what makes a device ask again. A bundle with no `to` comes from an older client and is accepted only while this device's own solicitation is in flight (`isSolicitInFlight`), so the doubtful case costs a diff, never a message. `to` is ADDRESSING, not secrecy - see [durable-rules](../durable-rules.md) on why the `recipients` field of `POST /send` must not be used for it.
