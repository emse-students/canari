...
### The hardware Back press had no owner, and Chromium decided it (2026-09-21)

`WryActivity` registers an `OnBackPressedCallback` that asks the WebView `canGoBack()` and calls
`goBack()` - the behaviour this app wants, because every mobile overlay IS a history entry
(`historyOverlayStack`), so stepping the history is closing the overlay. **`TauriActivity` turns it
off**: `override val handleBackNavigation: Boolean = false`, generated, not ours to edit.

With it off, nothing in this process handles the press. `targetSdk 36` means the system routes Back
through the predictive-back dispatcher, and the only thing registered on it is the callback
Chromium's WebView installs for its own history - on its own schedule, which is not a contract this
app can hold anyone to.

**MEASURED ON A Mi 9T, Android 16, `history.length 3` with `{"canariOverlay":1}` on top read through
CDP at the moment of each press** (`tools/cross-client-harness/navstack.mjs`, written for this):

| process | BACK 1 | BACK 2 | BACK 3 |
| --- | --- | --- | --- |
| one | list | posts | app exits |
| another, and again at 3 s, 8 s and 20 s after the landing | **app exits** | - | - |

Same build, same screen, same history, opposite outcomes - so it was never a settling race, and the
first reading of it as one is what cost the diagnosis two passes. `MainActivity` overrides the flag
back to `true`, and the press is decided here. **Eight consecutive trials afterwards, notification
tap and manual journey alike, gave `/chat` -> list -> `/posts` -> exit, one step per press** - no
double consumption, which was the risk of adding a second handler beside Chromium's.

`frontend/src/lib/mobile/androidBackNavigation.test.ts` is the only gate: none of this is visible at
compile time, and `gen/android` is regenerated by `tauri android init` and by a major Tauri upgrade.
The web half of the same defect - a landing that drained its own overlay entry - is in
[chat](modules/chat.md#and-the-re-verification-it-asked-for-says-the-mechanism-is-there-and-the-landing-destroys-it-2026-09-21).
