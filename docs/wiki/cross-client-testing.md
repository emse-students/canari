...
| PIN-11 | **The gate cannot be walked away from**, and it offers an exit that destroys nothing. Written 2026-09-05 from a user report: people who forget their PIN close the modal on every page rather than take the destructive reset | `W1 W2` | **`FAIL`** 2026-09-05, and all four assertions fall. `Escape` closes the gate; a backdrop click closes it (`backdrop: "clicked"`, so the event landed); it is gone after a client-side navigation to another route; and `exits: {signOut: 0, reset: 0, leaves: 0}` - in its default state the modal carries **no way out at all**, the reset and the account link both sitting behind a disclosure. Dirt is the estate's media 404s plus one browser advisory, both [backlog](backlog.md). [P1](backlog.md) |
