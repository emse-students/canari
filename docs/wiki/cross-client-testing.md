...
| MSG-2 | W2 -> A1 with the app foreground: no duplicate against the push | `+A1` | `pending` - BLOCKED on the ESTATE: A1 logs in and answers its PIN, then logs itself out before its device publishes a key package, so it holds no conversation to open (measured 2026-09-04 20:10). It used to die inside `realClick` as `no stable element for selector: text=Discussions`, recording NO verdict; `ensureChat` now refuses first and names which of signed-out / PIN-locked it is, and the command that lifts it (2026-09-04) |
...
| MSG-5 | Channel message converges on all three; no `masterSecret` in any payload | `+A1` | `pending` - BLOCKED on the ESTATE, not the product: the phone logs in and answers its PIN, then logs itself out before its device has published a key package, so it can be added to no group and its conversation list is empty. Measured 2026-09-04 20:10; cause and why the local branch has no third option in [backlog](backlog.md) |
...
| MSG-8 | Send to a BACKGROUNDED tab | `W1 W2` `+A1` | `pending` - BLOCKED on the ESTATE, not the product: the phone logs in and answers its PIN, then logs itself out before its device has published a key package, so it can be added to no group and its conversation list is empty. Measured 2026-09-04 20:10; cause and why the local branch has no third option in [backlog](backlog.md) |
