...
| HEAL-REVOKE-9 | Revoked while **OFFLINE**: the wipe is DEFERRED, not lost, and must not fire while the server is unreachable | `W1 W2` | `FAIL` on `ef9aaba8` - **and the half that matters PASSED**: severed in 4 ms, revoked (`stillAddressable: false` in 2 106 ms), and the state STILL PRESENT with `wipeRan: false`. A device that cannot ask does not conclude. The two unmet are `theWipeLandedAfterOneReload` and `itSaidSoInItsOwnLog`: the reload latched a dead cookie, stopped at `/login` and never learned it was revoked. **A LOGIN then wiped it - `identityKeys` 1 to 0, measured.** So the row asserts a RELOAD trigger the product deliberately does not have; the residue's lifetime is a P2 in [backlog](backlog.md) and the row's own specification is the open question |
