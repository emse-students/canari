...
| HEAL-NEW-11 | The responder is our own **W1, arriving LATE** - W3 goes amber ALONE, then W1 comes online | `+user` | `FAIL` - on `48b65d08`, and the ONLY HEAL-NEW row with a second unmet condition: `sameAccountEnrolled: false` (the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed) **and `wentAmberBeforeTheResponderArrived: false`**, which is the row's own premise and the one real question left here - the sidebar never went amber before the late responder arrived, so what the row means to measure did not happen. Re-run, and read that second condition. the order twin of HEAL-NEW-3, and the harder half: nothing is listening when the rows are minted, so something must ask AGAIN. Same final state as HEAL-NEW-3 or `FAIL` **Runner `healnew.mjs --row 11`** (2026-08-28) |
