...
| NOTIF-4 | Read on W1 while A1 is killed, W1 arriving at the salon AFTERWARDS: notification dismissed on A1 | `+push` | `PASS-DIRTY` - on `1f396ac7`, taken by a runner changed since, so a re-run is owed. a `PASS-DIRTY` on `1f396ac7` stands in the ledger, taken by a runner changed since |
...
| HEAL-REVOKE-5 | Revoked, then the account CHANGES a lot while it is away - groups created, groups deleted, messages sent - then it returns | `W1 W2` `+user` | `INVALID` - on `48b65d08`: `the victim could not be brought to an enrolled starting point, so there is nothing to revoke` - the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. the user's ask. The assertion is an EQUALITY against a fresh device enrolled in the same window (`HEAL-NEW-*` supplies the reference), not a repair count. Ending with MORE than the fresh device means the wipe kept something; ending with LESS means something survived just enough to poison enumeration **Runner `healrevoke.mjs --row 5`** (2026-08-28) |
| HEAL-REVOKE-6 | The same, where the revoked device is **the phone** | `+A1` `+user` | `pending` - the mobile half of the user's ask. **The instrument for it is now in: `deviceResidue()` is the AND of both halves and a runner asserts on it** (the web-only predicate was still what `healrevoke.mjs` imported until 2026-08-28, so this row would have passed on a dirty phone). What is left is the runner's victim being A1 rather than W3, which costs a re-enrolment per run - measured at no 2FA. A1's store is SQLite behind the native layer, not the WebView IndexedDB the web wipe clears, so "the wipe ran" is a different claim here and must be read from the native store |
| HEAL-REVOKE-7 | The **ORDER** of the return: the revoked device back BEFORE the other devices are online, and back AFTER | `W1 W2` `+user` | `INVALID` - on `48b65d08`, same cause as HEAL-REVOKE-5: no enrolled victim to revoke, because the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. Untried. Same equality across both orders, for the same reason HEAL-NEW-11/12/13 exist: a responder present from the start and one arriving late are different mechanisms. A difference in final state is a `FAIL`, a difference in time is dirt with a number **Runner `healrevoke.mjs --row 7 --order first` / `--order last`** (2026-08-28) |
| HEAL-REVOKE-8 | A group **DELETED while the device was revoked** must not return as a Sync row | `W1 W2` `+user` | `INVALID` - on `48b65d08`, same cause as HEAL-REVOKE-5: no enrolled victim to revoke, because of the device cap. Untried. HEAL-NEW-7 from the other side, and the sharper case: the returning device may hold a stale membership belief no fresh device would have. Three causes must stay separable - a server tombstone, a per-user dismissal, and an exit the DELETING device still owes **Runner `healrevoke.mjs --row 8`** (2026-08-28) |
| HEAL-REVOKE-9 | Revoked while the device was **OFFLINE**: the wipe is DEFERRED, not lost - and it must not fire while the server is unreachable | `W1 W2` | `pending` - **written 2026-08-28**, the user's own second question, and the half no row had ever asked. Two samples of one disk: while `cutHard` has the device really unreachable the state must still be PRESENT (`isDeviceRevoked` answers `false` when it cannot ask, because a transport failure is not an answer - a wipe here would mean every offline user loses their device), and after ONE reload with a network it must be empty. It ends there: the return equality is HEAL-REVOKE-5/7/8's subject, this row is the DEFERRAL. Measured by hand on W3 first, which is what showed it worth a row **Runner `healrevoke.mjs --row 9`** (2026-08-28) |
...
| --- | --- | --- | --- |
| HEAL-NEW-0 | The rig can mint a device the server has never seen, repeatably, on ONE 2FA | `+user` | **`PASS-DIRTY` on `48b65d08`** (2026-08-28 01:12 UTC) - the primitive the other ten rest on, and it holds: the wipe leaves no identity, no store and no cookie; the account comes back; a `device_id` the server had never seen is minted; the census carries it; and there is **no credential prompt** - the SSO session lives on `auth.canari-emse.fr` / `cas.emse.fr` and wiping the app's origin does not touch it, so a re-enrolling device fills no field. **THE LEDGER'S NEWEST ROW FOR THIS CELL IS A `FAIL` AND THE BOARD DELIBERATELY DOES NOT TAKE IT** (`03d015fd`, 2026-08-28 12:34 UTC, from a run of the primitive alone): it carries NO unmet condition and NO unobservable - it is dirt only, seven `unexplained` lines, of which six are ordinary `debug: [auth]` / `debug: [callback]` lines from the OIDC callback this row performs on purpose, and the seventh is Chrome's accessibility hint that a password form wants a username field - which is the PIN modal, not this row. So the classifier, not the primitive, is what that `FAIL` measures; the substantive verdict above stands. The gap is a `P3` in [backlog](backlog.md). That is why the eleven rows cost ONE 2FA and not eleven. The dirt is the bulk `[History] frame never read here and unreadable for good (past-epoch-application); will reconcile` a fresh device prints for every frame older than its own epoch - see the note below the table. **It reached this verdict only after three rig faults were fixed**; the three earlier `FAIL`s of the same night measured the instrument |
...
| HEAL-NEW-2 | Fresh device, **the PEER online** - a responder that is not us | `+user` | `FAIL` - on `48b65d08`, and on ONE unmet condition: `sameAccountEnrolled: false`. **The product healed** - `healed: true`, 10/10 ready in 6.1 s, `serverActive: 10` - so the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed, is the whole of the failure. Re-run. the `welcome_request` fallback, with the roster's other user answering **Runner `healnew.mjs --row 2`** (2026-08-28) **RE-RUN 2026-08-28 on `3b5cee35`: `FAIL`, and the cause is the ACCOUNT, not the code.** Every rig premise was met this time - the phone stopped at +0.5 s, the abandoned id purged (`true`, the first time ever), the fleet drained in 982 ms - and the device then healed ONE of ten rows in 600 s: 100 `welcome_request`s at the documented 60 s per-group cadence, 1261 throttles, two `externalJoin succeeded`, `ready 1 / syncing 9`. **`dm_device_group_memberships` settles it: the owner holds ELEVEN active groups and the peer shares TWO.** A responder can only serve a group it is a member of, so `expect: healed` over all ten is unsatisfiable here however correct the mechanism is. The row needs to assert the SERVABLE subset, the way HEAL-NEW-1 uses `either`. **IT NOW DOES (2026-08-28):** `expect: 'servableSubset'` asks the responder's OWN client which groups it is a member of, intersects that with this device's tiles by groupId, and terminates when every row IN the subset is ready - the rows outside are reported and not asserted. An empty intersection is `INVALID`, never `PASS`, at two independent guards (`servable.mjs`, pinned by `servable-selftest.mjs`). **The re-run waits on a DEPLOY**, because the join needs `data-conversation-tile` to carry the groupId and an undeployed bundle leaves it empty - which reads as an empty subset, so the row would refuse itself rather than lie. Until that re-run this cell is not a product verdict. The reconciliation itself ran and said so: `[HISTORY_RECONCILE] sweeping every group - no record of an earlier connection - new or restored store`, then `reconciliation pass complete` |
| HEAL-NEW-3 | Fresh device, **another device of the SAME user online** (W1) | `+user` | `INVALID` on `3b5cee35` (2026-08-28) - the row's own responder never reached the gateway, so it refuses instead of judging the product; the two earlier runs and the whole story are below. FIRST RUN was `FAIL` on `48b65d08`, `sameAccountEnrolled: false` alone, with `healed: true` and 10/10 ready: the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. Re-run. the responder is our own other device. This is the condition the user actually lives in **Runner `healnew.mjs --row 3`** (2026-08-28) **RE-RUN 2026-08-28 on `3b5cee35`: `FAIL`, and the FAIL was the rig's - the row had no responder at all.** `unlock W1 {"status":1}` then `fleet {"extra":[],"drainedInMs":961}`: W1 was running and SIGNED OUT on `/login`, `setTopology` could not restore a session, and the row then watched ten groups stay amber for 600 s (`syncing: 10`, `serverActive: 10`) and blamed the product. **RE-RUN AGAIN after the fix: `INVALID`, which is the correct verdict and the point of the fix.** `setTopology` now calls `bringToReady` (the preflight's own repair) and the fleet is asked with `awaitFleetMember`, the opposite polarity of `fleetBesides` - so a responder that never arrives refuses the row instead of producing a product verdict: `the row needs our own W1 online as the responder and the gateway never listed it within 94s`. **BLOCKED ON A HUMAN, NOT ON CODE:** W1's SSO session is gone, and `cas.emse.fr` answers "Authentification renforcee" - Esup Auth on the Pixel 6a, TOTP, or the NFC card. That is SETUP-4's 2FA, the one step no tool here can answer. The client's own PIN modal over `/login` was a separate P1, now fixed (`CHANGELOG.md`) |
...
| HEAL-NEW-11 | The responder is our own **W1, arriving LATE** - W3 goes amber ALONE, then W1 comes online | `+user` | `FAIL` - on `48b65d08`, and the ONLY HEAL-NEW row with a second unmet condition: `sameAccountEnrolled: false` (the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed) **and `wentAmberBeforeTheResponderArrived: false`** - which READ like a second, product question and is not: the note is `never went amber alone within 90s: {"panel":false}`, so the conversation panel never mounted at all, which is what a device refused its KeyPackage looks like. Same cause, twice. Re-run. the order twin of HEAL-NEW-3, and the harder half: nothing is listening when the rows are minted, so something must ask AGAIN. Same final state as HEAL-NEW-3 or `FAIL` **Runner `healnew.mjs --row 11`** (2026-08-28) |
| HEAL-NEW-12 | The responder is the **PEER W2, arriving LATE** - W3 goes amber alone, then W2 comes online | `+user` | `FAIL` - on `48b65d08`, `sameAccountEnrolled: false` alone, with `healed: true` and 10/10 ready: the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. Re-run. the order twin of HEAL-NEW-2. A `welcome_request` nobody heard is not a request that will be re-heard: whether it is re-issued on the peer's arrival is the row **Runner `healnew.mjs --row 12`** (2026-08-28) **SAME UNSATISFIABLE PREMISE as HEAL-NEW-2, measured 2026-08-28** - its responder is the PEER, which shares 2 of the owner's 11 groups, so a `healed` assertion over every row cannot be reached. **The row now asserts the servable subset** (2026-08-28, same mechanism as HEAL-NEW-2 above), so it is runnable for the first time - after the deploy that puts the groupId on the tile. Its own question is untouched by the narrowing: whether a `welcome_request` nobody heard is re-issued when the peer finally arrives |
...
| HEAL-NEW-15 | **Is the app usable while it heals?** N rows amber, and the user navigates and sends | `+user` | `FAIL` - on `48b65d08`, `sameAccountEnrolled: false` alone - `healed: true`, `usability.openedInMs: 26`, so the app WAS navigable: the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. Its `fleet.extra` also named two abandoned mints, the same debris. Re-run. the second half of the user's question. An amber sidebar that cannot be clicked, or a healed conversation that will not open, is a finding independent of whether the heal eventually completes - and a 10-minute heal is acceptable where 10 minutes of a frozen list is not **Runner `healnew.mjs --row 15`** (2026-08-28) |
...
**AND THAT STATE IS NOW RESTORABLE, AND ASSERTED BEFORE THE ROW SPENDS ANYTHING (2026-08-28).**
`healrevoke.mjs`'s `setTopology` was a SIXTH implementation of "bring a client up" - `launch.mjs
start` plus `pin.mjs`, neither of which can restore a session - so it reported W1 "already up" while
W1 sat on `/login`, and that is what produced the four verdicts above rather than the product. It now
calls `bringToReady`, the one repair that includes `login.mjs`, and the runner checks its ACTOR
**before minting the victim**: a row that cannot be performed records `INVALID` and spends no
enrolment, instead of burning one of the account's fifteen device slots to discover the actor is
signed out. The return-topology check does the same and carries the revocation and wipe evidence it
had already collected, rather than discarding a real measurement into a rig fault. `state.mjs`
gained the fourth value this needed: a client on `/login` reads `signedOut`, where it used to answer
`unknown (gate not on this route)` - the one answer that hid the blockage.
