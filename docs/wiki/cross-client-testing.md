...
| MSG-2 | W2 -> A1 with the app foreground: no duplicate against the push | `+A1` | `pending` - UNBLOCKED 2026-09-04 22:23. The phone used to log itself out before publishing a key package; it was being handed a `SameSite=Lax` cookie its engine discards, because the transport predicate asked only whether the ENGINE keeps a cookie and not whether the deployment can ISSUE one. Fixed in the client and the server ([sessions](sessions.md#the-credential-a-client-carries-itself)); verified on hardware - `refresh carries=stored credential` across an `am force-stop`, a key package published, tiles rendering. It used to die inside `realClick` as `no stable element for selector: text=Discussions`, recording NO verdict; `ensureChat` refuses first and names which of signed-out / PIN-locked it is |
...
| MSG-5 | Channel message converges on all three; no `masterSecret` in any payload | `+A1` | `pending` - UNBLOCKED 2026-09-04 22:23. The phone used to log itself out before publishing a key package; it was being handed a `SameSite=Lax` cookie its engine discards, because the transport predicate asked only whether the ENGINE keeps a cookie and not whether the deployment can ISSUE one. Fixed in the client and the server ([sessions](sessions.md#the-credential-a-client-carries-itself)); verified on hardware - `refresh carries=stored credential` across an `am force-stop`, a key package published, tiles rendering |
...
| MSG-8 | Send to a BACKGROUNDED tab | `W1 W2` `+A1` | `pending` - UNBLOCKED 2026-09-04 22:23. The phone used to log itself out before publishing a key package; it was being handed a `SameSite=Lax` cookie its engine discards, because the transport predicate asked only whether the ENGINE keeps a cookie and not whether the deployment can ISSUE one. Fixed in the client and the server ([sessions](sessions.md#the-credential-a-client-carries-itself)); verified on hardware - `refresh carries=stored credential` across an `am force-stop`, a key package published, tiles rendering |
...
| Id | What it asks | Needs | State |
| --- | --- | --- | --- |
| TYPE-1 | Typing on W1 shows on W2 within a second, and clears on stop | `W1 W2` | `PASS` 2026-09-04 22:41 on 0.16.3, clean - re-run after TYPE-3's teardown was repaired |
| TYPE-2 | It expires on its own after 6 s if the stop is never sent | `W1 W2` | `PASS` 2026-09-04 22:41 on 0.16.3, clean - the indicator expires on its own |
| TYPE-3 | Killing the tab mid-typing leaves no stuck indicator on the peer | `W1 W2` | `PASS` 2026-09-04 22:41 on 0.16.3, clean - `assertionHeld` AND `teardownRestored`, which is the pair this row now demands. The earlier `ERROR` was its own teardown: the restore spawned `pin.mjs` with this runner's directory as cwd, which became `archive/` when the runners moved, so it re-entered no PIN, left W1 locked, recorded `unlocked: false` and returned PASS anyway. Both halves fixed - the spawn path, and a verdict that may no longer claim PASS on a run that damaged what follows |
| TYPE-4 | An offline peer gets nothing, and nothing is replayed when it returns | `W1 W2` | `ERROR` 2026-09-04 22:41 on 0.16.3 - **INSTRUMENT, not product.** It PASSES standalone, reproducibly (`clean`, `whileOffline: ""`, `afterReconnect: ""`, peer offline in ~100 ms), and reproducibly ERRORs as `Inspected target navigated or closed` when driven by `run.mjs` inside the phase. Not the preflight and not a second page target - both were measured out. Diagnosis owed before this row is believed either way ([backlog](backlog.md)) |
| TYPE-5 | Channel typing, a different transport entirely (REST, not WS) | `W1 W2` | `PASS` 2026-09-04 22:41 on 0.16.3, clean - the REST transport. It failed first on `the community was never listed`, which was a STALE LITERAL and not a sidebar fault: `openChannel` defaulted to the string `Campagne de test`, and on this prod-copy estate that is a real community belonging to two production users. It now follows `VENUE`, like the runners already repaired |
