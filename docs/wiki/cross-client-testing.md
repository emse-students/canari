...
| HEAL-NEW-2 | Fresh device, **the PEER online** - a responder that is not us | `+user` | `FAIL` - on `48b65d08`, and on ONE unmet condition: `sameAccountEnrolled: false`. **The product healed** - `healed: true`, 10/10 ready in 6.1 s, `serverActive: 10` - so the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed, is the whole of the failure. Re-run. the `welcome_request` fallback, with the roster's other user answering **Runner `healnew.mjs --row 2`** (2026-08-28) |
| HEAL-NEW-3 | Fresh device, **another device of the SAME user online** (W1) | `+user` | `FAIL` - on `48b65d08`, `sameAccountEnrolled: false` alone, with `healed: true` and 10/10 ready: the account was at the server's 15-device cap, so `register-device` answered 400 and no KeyPackage was ever published - the instrument, now fixed. Re-run. the responder is our own other device. This is the condition the user actually lives in **Runner `healnew.mjs --row 3`** (2026-08-28) |
