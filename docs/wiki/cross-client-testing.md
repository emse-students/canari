...
| 16 HEAL | 33 | `0f06a4b3`, `48b65d08`, HEAL-NEW-3/11 `ebef7f3c`, HEAL-NEW-2/12/15 `038c7e8d`, HEAL-REVOKE-5/8 `0044a041`, HEAL-REVOKE-7 `edb8d7ab` | 10/33 taken: 3 `PASS`, HEAL-NEW-0/2/12/15 and HEAL-REVOKE-5/7/8 `PASS-DIRTY`; BOTH order pairs (2/12 and REVOKE-7) adjudicated EQUAL on the campaign page; -7 found a P1 (fixed `edb8d7ab`) and then FAILed once more on its RESIDUE before passing in a swept world; 1 and 3 owe an ungated re-run; HEAL-REVOKE-9 `PASS-DIRTY` on `da0ce2f2` after two `FAIL`s, the second of which found a P1 in the wipe (two `getStorage()` connections, one closed) |
...
| HEAL-REVOKE-7 | The **ORDER** of the return: back BEFORE the other devices are online, and back AFTER | `W1 W2` `+user` | `PASS-DIRTY` on `edb8d7ab` - **the pair is ADJUDICATED: both orders EQUAL**, `unmet: []` and `equalityGap: []` in each. Half B re-run on the fix: 11 rows / 11 ready / 0 syncing and the SAME eleven ids on both devices; wipe complete, `identityKeys: 0`, `canariDatabases: 0`; revoked through the panel, gone in 2692 ms; a tile opened in 151 ms. Owner at **3 of 15 slots**, unchanged. Dirt is five lines, all named, none new: the queued GRAINE stale-distribution rejoin, the queued kick+re-add, a coverage statement, and the wipe's own LOST frames, which this runner forgives on no observer by design. **Its FIRST attempt on the fix FAILed on that P1's RESIDUE** - see the campaign page |
...
| HEAL-REVOKE-9 | Revoked while **OFFLINE**: the wipe is DEFERRED, not lost, and must not fire while the server is unreachable | `W1 W2` | **`PASS-DIRTY` on `da0ce2f2`** - `unmet: []`, three samples of one disk. Severed in 4 ms; revoked (`stillAddressable: false` in 2 559 ms) with the state STILL PRESENT and `wipeRan: false` - a device that cannot ask does not conclude. A reload changed NOTHING and stopped at `/login`: no trigger fires for a page load that authenticates nobody. The LOGIN wiped it in 3 035 ms, `wipeFinished`, `storesSurvived: false`, and the PIN brought it back as a NEW device (`...mtfbhos7` for `...mtfadtkq`). Slots 4 -> 4. Link restored, PROVED by a request arriving in 58 ms. **Two earlier `FAIL`s, both worth having**: the first on a RELOAD trigger the product does not have (re-aimed, user 2026-08-30), the second on `noStoreSurvivedTheWipe` - a P1 in the wipe, fixed in `da0ce2f2`. Dirt: one LOST frame past its re-join, the queued GRAINE stale-rejoin P2, one `not-ready` WARN, and the revocation refusal the row itself provokes |
