...
| --- | --- |
| a shared post's photo reaches the unfurler (the user's own report, a Messenger card with an empty image box) | **ON PRODUCTION, AFTER THE STABLE DEPLOYS - AND NOWHERE ELSE.** The population was 11 of 11 association posts with an image previewing blank; re-run the same probe and it must be 0 of 11. **Dev cannot answer this**: its copy strips every media reference and its bucket holds 0 objects, so there is nothing to render and no way to tell a fixed preview from a broken one ([dev-environment](infrastructure/dev-environment.md#the-consequence-nobody-had-written-down-a-media-path-cannot-be-rehearsed-here-2026-09-22)). One `curl` of `/api/posts/<id>/preview-image` from inside the container, and one real Messenger paste, settle it |
