...
| **send the DSI the one request that carries every name at once - IT IS WRITTEN OUT, copy it** - three records to create, `canari.emse.fr` to reassign, the certificates, the firewall confirmation and the account question. The certificate question that used to block it was ANSWERED by measurement on 2026-09-24 and removed from the request. Every name costs a ticket, so a second request is a second wait | 1 message | [estate-migration](infrastructure/estate-migration.md#the-request-written-out---copy-it-do-not-rewrite-it) |
...
| **ask the gala team whether 160 MB on the shared host may go** - a runner workspace holding the only surviving checkout of `emse-students/refonte-gala`, a repository that now answers `404`; the repository that looks like its successor does not contain that commit. Nothing runs from it and nothing points at it, so this is not a technical question but somebody else's archive | 1 conversation | [estate-migration](infrastructure/estate-migration.md#the-host-was-emptied-before-the-move---2026-09-24) |
| **take ONE Authentik dump by hand, or cut a stable that carries the backup fix** - the repair is written and proven, but production only redeploys on a STABLE release, so until one ships the estate keeps adding nights to the 93 it already has with no identity backup at all. An agent cannot close the gap in the meantime: taking a retained copy of the identity database was refused here as personal-data handling, and that refusal is correct rather than something to route around. The one command, from the `canari` box: `ssh miconnect@10.0.0.7 authentik-pg-dump > ~/backups/authentik_db-$(date +%Y%m%d).sql.gz` | 1 command, or 1 release | [backup](../../infrastructure/backup/README.md) |
