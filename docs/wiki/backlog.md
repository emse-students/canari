...
- **Edge caching with a short `s-maxage`, and a purge on deploy. SHIPPED 2026-09-17**, both halves in
  ...
  **AND IT IS SERVING, WHICH `SHIPPED` DID NOT SAY.** `SHIPPED 2026-09-17` was a statement about the
  COMMIT - a merged fix is not a shipped fix, twice over in this file - and it was read once as
  though production were already answering from the edge. Production now is: the same URL answers
  `cf-cache-status: HIT` in **~54 ms** and, after a purge, `MISS` in **~96 ms**, measured back to
  back on 2026-09-17. The **controlled pair is the point**: a single absolute number cannot tell a
  warm edge from a fast origin, and the ~42 ms between two fetches of the same bytes can only be the
  round trip that no longer happens.

  **IT IS NOT THE SAME NUMBER AS THE ~80 ms PREDICTED FROM THE USER'S FIREFOX, AND SAYING SO WOULD BE
  THE MISTAKE.** Same order, different measurement: the 94 ms of 1342 read from that profile is an
  origin round trip seen from where the user sits, and this pair was taken from this workstation. The
  two do not confirm each other and neither should be quoted as the other's evidence - what this pair
  establishes is that the edge is SERVING, which is the claim `SHIPPED` was wrongly read as making.
...
### G2 - ONE DISPLAY NAME, TWO OPPOSITE PRECEDENCES - the residue of the launch replay

The replay itself and its slug title shipped 2026-09-18 (`CHANGELOG.md`, and
[mobile](frontend/mobile.md#what-the-shade-cannot-remember) for the mechanism). Two things reading
it turned up are NOT fixed, and only they belong here.

1. **The server and the client disagree on how to build a name.** `formatDisplayName`
   (`display-name.ts:12`) prefers `displayName` over first+last; `formatProfileDisplayName`
   (`displayName.ts:92`) prefers first+last over `displayName`. **Two opposite precedences for one
   question**, so a push and a locally-raised notification can title the same person differently
   even when both resolve correctly. One of the two is wrong and nothing here says which; the fix is
   to pick a precedence, state it where the type is declared, and delete the other.
2. **A SALON message is covered by neither half of the launch fix**, for the same missing field as
   `G1`'s first residual - the channel push payload carries no timestamp, so the builder can
   identify no message of its own and the record never guesses. **One fix closes both**: do not
   re-derive it here.
...
**THE JOB PASSED FOR THE FIRST TIME ON 2026-09-16**: `ok GitHub holds no open Dependabot alert for
this repository.` **`GITHUB_TOKEN` cannot read Dependabot alerts at any permission level** - there is
no `permissions:` key for them at all, and `security-events` is code scanning, a different thing - so
the 403 was never a workflow setting away from lifting. A fine-grained token with
`Dependabot alerts: Read-only` on this repository alone now sits in `DEPENDABOT_ALERTS_TOKEN`, read
at `.github/workflows/scheduled.yml:183` as
`GH_TOKEN: secrets.DEPENDABOT_ALERTS_TOKEN || secrets.GITHUB_TOKEN`. **The zero it prints is now a
measured zero rather than an unread one**, which is the whole point of the loud failure above. The
job starts failing loudly again on the day the token lapses, and that is correct.
