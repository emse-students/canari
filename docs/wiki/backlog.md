...
| **SEND THE MAIL TO THE DSI, AND A CORRECTION IS OWED IN IT** - 12.9% of returns from real browsers carry no `code` (the 21% already mailed counted robots), and no change on our side can make `cas.emse.fr` send one. The loop it caused is fixed; the failures are not. The text is written and needs no editing - it now also carries three questions on their `requiredAttributes` gate, a `jq` check on their file, and a request to drop the `password` grant and rotate the client secret | 1 mail | [authentik](infrastructure/authentik.md#the-follow-up-to-send) |
| **PKCE on the `cas-emse` source: `none` -> `S256`, DELIBERATELY NOT DONE.** It is unrelated to the loop, it cannot be verified without completing a real CAS login, and its blast radius is every login of all seven clients. Flip it, then log in once immediately, and roll back to `none` if that login fails - not the other way round | decision, then 1 login | [authentik](infrastructure/authentik.md#a-share-of-cas-returns-carry-no-code-and-our-login-page-turned-that-into-a-livelock---2026-09-08) |
...
## P2 - about 73 logins a week fail at CAS and NOTHING reports it, re-measured 2026-09-09

**Substance, and the only copy.** 52 empty returns FROM REAL BROWSERS in 120 h is **~10 a day, ~73 a
week** - the first figure written here, "71 in 96 h, ~121 a week", counted robots and was WRONG. Every one
...
([authentik](infrastructure/authentik.md#a-share-of-cas-returns-carry-no-code-and-our-login-page-turned-that-into-a-livelock---2026-09-08)).
...
stops quietly on a page, so nothing at all points at the 73.
...
`?error=access_denied` is a user cancelling and is not this. **And it must EXCLUDE every non-browser
user agent, this project's own probes first of all** - the counter runs on a log that a reproduction
script writes into, and the first figure published here was wrong for exactly that reason
([durable-rules](durable-rules.md), the Authentik section).
The rate is the number worth alerting
on (12.9% now), never the absolute count, because the denominator is a live population and moves with
