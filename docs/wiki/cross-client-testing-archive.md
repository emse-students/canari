# Cross-client testing - the LITHIUM board, archived 2026-09-03

**EVERY VERDICT BELOW WAS TAKEN ON A RIG THAT NO LONGER EXISTS, AGAINST PRODUCTION, WITH A LEDGER
THAT IS GONE.** This page is the record of what the campaign had measured between 2026-08-14 and
2026-08-30. It is not the board - the board is
[cross-client-testing](cross-client-testing.md), and it starts from zero.

**Why it was reset rather than carried over** (decision 12 of the
[workflow migration](workflow-migration.md#2-the-twelve-decisions---decided-not-to-be-relitigated),
taken with the user on 2026-09-02). The machine this repository now lives on, `OXYGEN`, was
reconstituted from a handoff bundle collected on the previous machine, `LITHIUM`, **without
`-WithRig`**: 131 KB, no `chrome-w1` / `chrome-w2` profiles, no `results.ndjson`, no `apk/`, no
`a1-baseline/`. So every cell below has lost the ledger that justified it.

**That is not a bookkeeping problem, it is an evidence problem.** This campaign's own methodology
says a verdict is believed because the run behind it can be read - the dirt it collected, the lines
it classified, the build it named. A cell whose ledger is gone is a claim with nothing behind it,
and the honest thing is not to carry it forward but to say so once and re-measure. Three of these
verdicts were already marked NOT settled before the loss (DEL-10, COMM-8, COMM-23), which is the
same problem arriving by a different road.

**Two further things changed under it**, either of which would have invalidated a large part of the
board on its own:

- **The target moved from PRODUCTION to the LOCAL estate.** A row that measured production measured
  a different system: a different origin, a different cookie policy (`SameSite=Lax` without
  `Secure` locally, `SameSite=None; Secure` in production), a different database.
- **The accounts changed twice.** The rows were taken on real people's accounts, then the campaign
  moved to dedicated Authentik users on 2026-09-02, and the rig restarts on fresh ones again.

**Read a cell here as: "this mechanism held once, on this build, on a rig nobody can re-open."**
That is worth keeping - it says where to look first when a re-run disagrees - and it is worth
exactly nothing as a gate.
...
| 16 HEAL | 33 | `0044a041`, `edb8d7ab`, `da0ce2f2`; HEAL-NEW-2/12/15 `038c7e8d`, -3/11 `ebef7f3c`, -0 `48b65d08` | 10/33 taken: 3 `PASS`, 7 `PASS-DIRTY`; both order pairs adjudicated EQUAL; two P1s found and fixed; HEAL-NEW-1 and -3 owe an ungated re-run |
...
| HEAL-REVOKE-7 | The **ORDER** of the return: back BEFORE the other devices are online, and back AFTER | `W1 W2` `+user` | **`PASS-DIRTY`** on `edb8d7ab` - both orders EQUAL, 11/11 each, gap empty |
...
| HEAL-REVOKE-9 | Revoked while **OFFLINE**: the wipe is DEFERRED, not lost, and must not fire while the server is unreachable | `W1 W2` | **`PASS-DIRTY`** on `da0ce2f2` - kept while severed, wiped at login in 3.0 s |
