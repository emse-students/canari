...
`Object.create(null)` answers the other half and stays wherever such a map is built: a
prototype-less object has nothing to walk into. **What it does not stop is the map CARRYING a key of
that shape**, which then travels to every reader of the poll and into JSON - which is why
