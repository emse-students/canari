 ...
 * knows a uuid, so the refusal belongs at the mutation - here, adding a member to a group.
 ...
 * actor" would leave the blocker able to pull the person they blocked into a group, which turns a
 * closed door into a one-way channel.
 *
 * DUPLICATED IN `social-service` ON PURPOSE - see the note at the head of `service-urls.ts`:
