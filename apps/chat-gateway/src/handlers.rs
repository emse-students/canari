...
use crate::models::{Claims, AuthParams, WebSocketMessage, Recipient, RatchetTreePayload, process_incoming};
...
pub async fn ws_handler(
    ...
    match decode::<Claims>(&params.token, &key, &validation) {
        Ok(token_data) => {
             ...
             ws.on_upgrade(move |socket| handle_socket(socket, state, token_data.claims.sub, device_id))
        },
...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>, user_id: String, device_id: String) {
    ...
    if let Ok(mut con) = state.redis_client.get_multiplexed_async_connection().await {
        ...
        if let Ok(msgs) = pending {
            ...
            let _: Result<(),_> = redis::cmd("DEL").arg(&welcome_key).query_async(&mut con).await;
        }
    }
