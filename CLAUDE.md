...
* \[x\] **N8 DONE** (pushed 28226cf): Canari public API serves bios as Markdown in TWO fields (`description` legacy + `bioMarkdown` current); Canari renders BOTH via `@humanspeak/svelte-markdown`. Portail was running only `description` through an HTML sanitizer + `{@html}`, so MD showed literally and `bioMarkdown` was ignored. Fix: ported Canari's ProfileBioMarkdown (same lib + gfm/breaks + single-newline->hard-break normalize), new `ProfileBioMarkdown.svelte`/`BioLink.svelte`/`markdown.ts`; EntityDetail renders both fields; deleted unused html.ts sanitizer + test + the no-at-html-tags eslint override. Mentions/hashtags/hljs are Canari-only, omitted.
* \[x\] **N6 DONE** (pushed 105b33a): comment sweep found all code + HTML comments in `src/` already English (nothing left). While there, fixed an N3 i18n miss - the association/list `[handle]/+page.ts` load fns threw hardcoded FR `error(404, "...introuvable")`; added `error_association_not_found`/`error_list_not_found` to fr+en, both throw `m.*()` (locale resolves client-side in SPA). `site.ts` brand strings (SITE_NAME/TAGLINE) stay FR by design (documented deferral).
