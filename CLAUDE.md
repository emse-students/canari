...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Recreates the hand-made assos poster (bubbles = asso color+logo+president photo+roster, grouped by theme) as a data-driven, re-editable PDF via the existing snapdom pipeline. Decisions ACTED: managed category TABLE (not enum) + 1 primary category/asso + per-bubble override in editor; access = `GlobalAdminOrBdeSuperAdminGuard` (global admin + BDE super-admin); project = persisted JSON layout (photos/members re-resolved at render -> always current). Phases: P0 foundations / P1 generator+static render+PDF / P2 drag-drop editor / P3 doodles+themes+free text. **P0 BACKEND DONE (38cf58b4), social-service:** entity `AssociationCategory` (+ migration `021`, seeds 5 poster sections) + `Association.categoryId` loose uuid (nulled on category delete) exposed via list spread + `UpdateAssociationDto.categoryId` (blank->null); entity `PosterProject` (jsonb `layout`, + migration `022`) + `PosterService` CRUD. Two controllers `AssociationCategoriesController` (`associations/categories`, GET public, writes guarded) + `PosterController` (`associations/poster`, all guarded) registered BEFORE `AssociationsController` so literal routes win over `:id` (reuses nginx `/api/associations`, no infra change). GOTCHA: prod applies `.sql` migrations manually (`synchronize` only in dev); each schema change needs BOTH the entity AND a numbered idempotent `.sql`. **P0 FRONTEND DONE (this commit):** typed API client in `frontend/src/lib/associations/api.ts` (`AssociationCategory`/`PosterProject` types + `categoryId` on `Association`/`UpdateAssociationPayload` + category CRUD/reorder + poster CRUD); `/admin/carte` hub (`+page.svelte`: category manager with add/rename-on-blur/delete/up-down reorder + poster-project list create/open/delete) gated on global-admin OR BDE-super-admin (mirrors document-reviewers pattern); stub editor `/admin/carte/[id]/+page.svelte` (loads project, "coming soon" until P1); dashboard card + admin nav item (Map icon, `carte_card_label`); `categoryId` `<select>` picker in `EditProfileTab.svelte` (regular assos only, loads categories on mount, saved via `categoryId: editCategoryId || null`); FR/EN i18n (`carte_*`, `asso_edit_category_*`). GOTCHA: frontend `format` script is `oxfmt -c ../oxfmt.json --write .` - running bare `oxfmt` uses defaults (double quotes) and corrupts the file; always pass `-c ../oxfmt.json`. **NEXT = P1:** generator (auto-populate one bubble/asso pre-grouped by category) + static render (bubbles + president polaroids + themed directory) + PDF via snapdom (`pdfRaster.ts`), filling the `/admin/carte/[id]` editor page.
