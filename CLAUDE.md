...
* \[~\] **Carte - FEEDBACK PASS (UNCOMMITTED on disk, classifier down before gates+commit).** Resume = run `cd frontend && bun run check && bun run lint && bun run format` then commit these files: `apps/social-service/src/migrations/021_association_categories.sql`, `frontend/src/lib/carte/layout.ts`, `frontend/src/lib/components/carte/PosterCanvas.svelte`, `frontend/src/routes/admin/carte/[id]/+page.svelte`. Changes (all from user feedback 2026-07-17): (1) migration 021 no longer seeds categories - deleted the INSERT block; categories are 100% admin-managed now (deleted ones were reappearing because the re-applied seed re-inserted a slug that no longer conflicted). (2) Full-page overlay now has a floating exit button (the header toggle was hidden BEHIND the `fixed inset-0 z-50` overlay). (3) Stage div got `isolation:isolate` so "send to back" (negative z) no longer sinks a bubble behind the bg image. (4) Blob unit redesigned: `CARD_HEIGHT=430` (was square), blob drawn FIRST (behind) with bureau polaroids fanned over its TOP arc drawn AFTER (in front - user explicitly wants blob behind, polaroids in front, overlap intentional); hero logo `LOGO_SIZE=104`; asso NAME moved to a band BELOW the blob (wraps + length-adaptive `nameFontSize()`, no longer clipped inside the blob). Geometry: `UNIT_CX`, `BLOB_CY=172`, `BLOB_SIZE=210`, `RING_RADIUS=125`, top-arc angle `-PI + ((i+0.5)/n)*PI`. (5) Bureau polaroid captions wrap 2 lines (`-webkit-line-clamp`) + `polaroidFontSize()`. (6) Directory `<aside>` now flex-column with a clipped body + a `$effect` that imperatively shrinks `dirContentEl.style.fontSize` (item text in `em`) until the whole roster fits - fixes overflow clipping for big rosters. (7) Seed: `fitSeedScale()` computes the largest scale (0.2..0.6) that fits the whole roster inside the A2 frame (was fixed 0.6 -> ~30 assos overflowed `STAGE_HEIGHT` and got clipped = "missing assos"); `mergeBubbleLayout` now also clamps legacy saved x/y into the frame; `resetBubble` resets to seed scale not 1. BROWSER-VERIFY still pending (snapdom can't run on host). Below = prior Step 3 detail (still valid geometry-wise except the numbers above supersede it).
