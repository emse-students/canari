...
iOS is ~80% a port of the Android native push stack: `gen/apple/Sources/canari/canari_push.mm` mirrors `CanariFirebaseMessagingService.kt`; `src-tauri/src/mobile/ios_ffi.rs` is a C-ABI twin of the Android JNI. The hand-maintained `canari.xcodeproj/project.pbxproj` is the build source of truth (NOT `project.yml`/xcodegen) and is edited BY HAND (targets, resources, variant groups all added directly to the pbxproj - no Mac/Xcode needed, the CI macOS runner builds it). Everything now builds+signs+exports end-to-end in CI.
...
SHIPPED (WP-iOS-6/7/10/11 - all code + CI wired in git, no Mac step remains; only physical-device runtime checks are left, see [device] list):
* **WP-iOS-6/7 NSE (wired + built green):** decrypted, rich iOS notifs. `CanariNotifications` is a real target in the pbxproj (embedded as a Foundation Extension, links `libapp.a`+`Security`+`UserNotifications`), `NotificationService.swift` reuses the Rust decrypt FFI, App Group `group.fr.emse.canari` mirrors `mls.bin`/`push_context.json`/`channel_keys.json`/push secret (`CanariMirrorPushStateToAppGroup`). Backend `buildInternalApnsRequest` gives channel/social pushes `mutable-content`. CI builds+signs+exports the appex.
* **WP-iOS-10 (localized prompts - wired this session):** `InfoPlist.strings` (fr/en, the 4 `NS*UsageDescription` keys) are now a `PBXVariantGroup` in the pbxproj, added to the app Resources phase, `fr` added to `knownRegions`; base Info.plist stays FR as fallback. Done by hand in the pbxproj (same as the NSE) - NO Mac needed.
* **WP-iOS-11 CD (GREEN, run 29857076911):** `ios-release.yml` builds a signed `.ipa` on a free public macOS runner and attaches it to the GitHub Release `vX.Y.Z` (real `workflow_run` trigger; Upload-to-Release SKIPPED on manual `workflow_dispatch`). The 5 build fixes are in git (Info.plist un-resourced; prebuild `libapp.a`; `rustup target add aarch64-apple-ios`; link `BackgroundTasks.framework`; split archive/manual-export because Tauri's auto ExportOptions can't map two manual profiles). Durable facts: two NAMED profiles `Canari` (app) + `CanariNotifications` (NSE) must match the pbxproj `PROVISIONING_PROFILE_SPECIFIER` exactly, from secrets `APPLE_PROVISIONING_PROFILE`/`APPLE_PROVISIONING_PROFILE_NSE`; team "Les Rootz" `4CLNB8SR6L`; profiles expire 2027-07-11. Benign warning to ignore: `ld: ... built for newer iOS version (26.2)` (runner SDK). Signing/secrets facts live in this state (secret-setup guide deleted as transitional).
* **Version sync (this session):** NSE `MARKETING_VERSION`/`CURRENT_PROJECT_VERSION` (pbxproj) + NSE Info.plist (`$(MARKETING_VERSION)`) now track the app version; `scripts/bump-app-version.sh` patches the pbxproj on every bump so the appex/parent version check passes. Killed the cosmetic `CFBundleShortVersionString ... must match parent` warning.

[device] runtime checks (need a physical iPhone - NOT a Mac/CI task, genuinely un-automatable):
* WP-iOS-1/2/4/8 native push (login scheme, heartbeat, BGTask, FCM token refresh); WP-iOS-6/7 decrypted banner (DM + group + `#channel`); WP-iOS-10 EN prompts on an English-locale iPhone.

STORE PUBLISH (CI fully wired + all 4 secrets set; v0.9.21 verified end-to-end this session):
* **iOS** `ios-release.yml` runs `xcrun altool --upload-app` (App-Store-signed `.ipa` -> App Store Connect/TestFlight; a raw `.ipa` is NOT sideloadable from a Release, unlike Android's `.apk`). Secrets `APP_STORE_CONNECT_API_KEY_P8` (base64 `.p8`), `APP_STORE_CONNECT_KEY_ID`=`HAQRC85NLH`, `APP_STORE_CONNECT_ISSUER_ID`=`b0bcfe88-4988-4f78-a80d-84c17642c7fc` are set. Step SKIPS-not-fails if secrets vanish. **Manual one-time still required:** create the app record in App Store Connect, fill the store listing/screenshots/privacy, and submit the first build for review (altool only UPLOADS the binary; it does NOT auto-submit to the public App Store - that needs the human review + metadata, unautomatable here).
  * **Gotcha (v0.9.21, fixed 7e6b23a5):** Apple rejected the upload with `STATE_ERROR.VALIDATION_ERROR` because the 1024x1024 marketing icon (`AppIcon-512@2x.png`) had an alpha channel/transparency - Apple requires that icon fully opaque. Compounding bug: `altool` exited 0 even though its own output said `UPLOAD FAILED`, so the workflow printed a false "Uploaded to TestFlight" success. Both fixed: all `AppIcon.appiconset` PNGs (+ their canonical source `src-tauri/icons/ios/`) regenerated from `src-tauri/icons/Canari.png` (master logo, opaque navy square) with alpha flattened via `sharp`; the workflow now captures the altool transcript, checks its exit code, and greps for altool's own failure markers before declaring success. Next release should be watched to confirm the icon fix actually clears App Store Connect validation.
* **Android** `android-release.yml` `r0adkll/upload-google-play` publishes the AAB to **`production`** only (uploads + fully rolls out). There is deliberately no `internal`-track step: Google refuses re-uploading the same versionCode to a second track in the same job (confirmed on v0.9.21 - "Version code 9021 has already been used"), so it was removed rather than left as a guaranteed-fail best-effort step. Gated on a `Check Google Play secret` step (the `secrets` context is ILLEGAL in a step `if:` - use an output step). Secret `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` set (`canari-ci@canari-496310.iam.gserviceaccount.com`). Verified working on v0.9.21 (bundle uploaded + committed). **Manual one-time still required:** create the app + first production release in the Play Console (Google refuses API uploads before the first manual release) - already done since v0.9.21 published successfully.
* **Update-target repo vars still unset (correctly):** neither `VITE_ANDROID_PLAY_STORE_URL` nor `VITE_IOS_APP_STORE_URL` is configured yet - user has no Play Store URL yet, and the iOS App Store numeric id the user pasted (`apps.apple.com/fr/app/facebook/id6793060521`) is Facebook's real listing, not Canari's (flagged back, not used). Do not set either until the user supplies Canari's real listing URL/id.
* **In-app "update" prompt (`AppUpdateModal` + `appVersion.ts`):** Android update target = Play Store when repo var `VITE_ANDROID_PLAY_STORE_URL` is set, else APK-download fallback (no regression); iOS target = `VITE_IOS_APP_STORE_URL`, and `hasConfiguredUpdateTarget()` HIDES the iOS prompt entirely until that var is set (iOS has no sideload fallback -> never show a dead action). Both vars injected into the release `.env` (empty by default). Set them (`gh variable set`) once each store listing is live: Android `market://details?id=fr.emse.canari`; iOS `itms-apps://apps.apple.com/app/id<NUMERIC_ID>` (numeric id only exists after the App Store record is created - user still owes it).
...
* \[x\] **WP-COT-1 (data model + tag derivation, migration 025):** `association_products` gained `variantKey`/`variantLevel` (NULL = single-tier, back-compat). `deriveCotisationTag(slug, mode, now?, variant?)` appends `-${variant}` before the academic-year suffix.
* \[x\] **WP-COT-2 (purchase logic: XOR switch + pay-the-difference, migration 026):** `association_products.memberPriceTag` - when set, `amountCentsMember` applies iff the buyer holds THAT specific tag (no fallback to generic `isBuyerCotisant` when set). `resolveGrantTag` now passes `product.variantKey` into `deriveCotisationTag` (previously all tiers of an asso collapsed onto the same base tag - fixed). Fulfillment wraps the grant in `productRepo.manager.transaction(...)` and calls new `revokeSiblingTierTags` (products.service.ts), which revokes the user's tag(s) for the asso's *other* `membership`-product variantKeys via new `UserTagService.revokeByName(userId, tagName, manager?)`; `grantOrRenew` also gained an optional `manager` param to join the same transaction. Same-tier rebuy-while-active still blocked (existing paidCount+hasActiveTag check, now tier-aware since resolveGrantTag is fixed); sibling-tier purchase allowed (paidCount is per-product). Tests in `products.service.spec.ts` (memberPriceTag pricing incl. no-fallback case, tier-specific grant tag, rebuy-blocked/switch-allowed, XOR revoke incl. single-tier no-op). Wiki: `docs/wiki/cotisations.md` "Multi-tier cotisations" section.
* \[x\] **WP-COT-3 (generalized gating, migration 027):** `association_products.requiredTags` (`text[]`, OR semantics) gates purchase on ANY listed tag - not scoped to the owning association (a form/other-asso tag works) - and takes precedence over `membersOnly` when set. Fixed a real bug in the process: `isBuyerCotisant` used to check only the base, un-suffixed tag, so `membersOnly` silently failed to admit sibling-tier cotisants of a multi-tier asso (e.g. Le Cercle); it now enumerates every distinct `variantKey` among the asso's `membership` products via new `tierVariantKeys()` (`cotisation-tag.util.ts`) and passes if the buyer holds ANY tier's tag - kept deliberately generic (not tier-specific) so the Cercle `balance_topup` recharge stays open to every forfait. `/products/all` (`ShopProduct`) now also returns `viewerActiveTier` (the specific `variantKey` the viewer holds, `null` if none/single-tier) alongside `viewerIsCotisant`. Tests in `products.service.spec.ts`.
* \[x\] **WP-COT-4 (inbound API Cercle->Canari):** `GET /api/public/cotisant-status?assoSlug=&sub=` added to the existing `PublicController` (`apps/social-service/src/public/public.controller.ts`), gated on `X-Api-Key` vs `CERCLE_API_KEY` (timing-safe compare, same pattern as core-service's `ExternalController`) - this controller's other routes stay intentionally unauthenticated, only this one checks. Throttled 20 req/min via `@nestjs/throttler` (`ThrottlerModule.forRoot` added to social-service `app.module.ts`, new dep in `package.json`/lockfiles). Logic lives in `ProductsService.getCotisantStatusBySlug` (reuses the `isBuyerCotisant` tier-enumeration pattern, returns `{ isCotisant, tier, expiresAt }`); new `UserTagService.getActiveTag` surfaces the tag row (not just a boolean) for the expiry. `CERCLE_API_KEY` wired into `.env.example` + both compose files' `social-service` block. No nginx change needed (`/api/public/` is already a catch-all, no `auth_request`). Wiki: `docs/wiki/cotisations.md` new "Cercle integration" section documents BOTH directions (outbound webhook contract + this endpoint).
