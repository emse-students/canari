...
- \[x\] **WP-XP-7 Unified rich notif grouping (this commit):** iOS app-alive path now uses per-conversation `threadIdentifier = groupId` (was flat `@"canari_messages"`) matching the NSE path (WP-iOS-7 already did it for killed-app). Adds `subtitle` (sender name inside group chats) and `summaryArgument` (iOS 15+ group-summary text) to both `CanariShowLocalNotification` and `NotificationService.swift`. `CanariPushCancelMessageNotifications` updated to clear per-conversation notifs via deepLink prefix check. Android `MessagingStyle` + `GROUP_KEY_MESSAGES` summary already provided the same model since day one. [device] verify stacking + summary on a physical iPhone.
