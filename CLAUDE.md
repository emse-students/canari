...
* \[~\] **WP-iOS-11 (CD, Léon owns):** `ios-release.yml` full pipeline compiles on Léon's Mac, still `workflow_dispatch`+`force` gated. Remaining: set the Apple secrets (`APPLE_*`, `GOOGLE_SERVICE_INFO_PLIST`, `JWT_SECRET`/`BASE_URL`) as GH secrets + a TestFlight upload step; the NSE needs its own provisioning profile. Guide: `docs/wiki/infrastructure/ios-ci-cd-setup.md`.
