...
* \[x\] **WP-iOS-5 biometric plugin cleanup (no [device]):** rewrote `patches/tauri-plugin-keystore/ios/Sources/KeystorePlugin.swift` off the upstream UniMe sample. Keychain item is now Canari-branded (`kSecAttrService="fr.emse.canari"` + `kSecAttrAccount="canari_biometric_user"`, mirroring what `biometric.ts` passes) via a single `baseQuery()` helper shared by store/retrieve/remove (was a triplicated hardcoded `com.impierce.identity-wallet.unime-dev` account). `retrieve` builds an explicit `LAContext` (French `localizedReason`, `interactionNotAllowed=false`) + `kSecUseOperationPrompt`; the stored item's `.userPresence` access control makes `SecItemCopyMatching` drive the Face ID/Touch ID prompt through that context. Added Swift doc comments to all three methods. `biometric.ts` comments/JSDoc/TODO de-Androidised (now describe both the Android Keystore path and the iOS keychain `.userPresence` path). Gotcha: the **Android** side (`KeystorePlugin.kt`) still uses alias `unime_dev` - deliberately NOT touched (renaming it orphans every already-enrolled Android user's key); iOS was safe to rebrand only because iOS biometric never shipped. svelte-check 0/0.
* \[x\] **WP-iOS-8 launch-time FCM token fetch (no [device] for the code):** `canari_push.mm`'s `didReceiveRegistrationToken` fires ONLY when the token changes, so a token that rotated while the app was killed never re-registered. Added a launch-time force-fetch (`[[FIRMessaging messaging] tokenWithCompletion:]`) in `CanariPushSetup`, mirroring Android `MainActivity.onCreate`'s `FirebaseMessaging.getInstance().token` read: best-effort, once per cold start, persists + re-registers on success. Extracted the shared `CanariPersistFcmToken(token)` helper (write `fcm_token.txt` + `CanariRefreshTokenOnBackend` when ctx+secret exist) so the delegate callback and the launch fetch share one code path (was inline-only in the delegate). Note: unlike Android (which only writes the file + SharedPrefs and lets the Rust `get_fcm_token` cmd read it later), the iOS path also eagerly re-registers on the backend inside the same helper - kept that existing behaviour. svelte-check 0/0 (Obj-C++ only, not compilable without a Mac; verify token re-registration after a killed-app rotation on a signed build). [device] optional runtime confirm.
