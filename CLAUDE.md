...
* \[~\] **N7 CD via GitHub Secrets - CODE DONE + PUSHED (f135b0e), pending user-only setup.** deploy.yml now wipes `~/portail-etu` and rewrites `.env` from 4 GitHub Secrets each push (via `env:`, masked; Bun auto-loads `.env`; empty values fall back to public defaults / disable avatar proxy). Dropped dead sharp (rollup external + `bun install ... sharp` in deploy.yml/release.yml) + stale commented Setup-Bun block. `.env.example` is the canonical secret-name list. **REMAINING (user, cannot automate):** in repo Settings > Secrets, create `PUBLIC_CANARI_URL`, `GALLERY_API_URL`, `GALLERY_API_KEY`, `PORTAL_URL`; rotate `GALLERY_API_KEY` on MiGallery then paste new value into the Secret. Only `GALLERY_API_KEY` is truly secret; the 3 URLs have code defaults (safe if left empty). Verify first post-setup deploy serves avatars.
