...
STORE PUBLISH (CI fully wired + all 4 secrets set; verified end-to-end on v0.9.21 and again on **v0.10.1** - the WP-XP-1/2/3 release: iOS IPA -> TestFlight, Android AAB -> Play production, both green):
...
* **OPEN - backend Docker build broken on main (pre-existing, NOT WP-XP):** `oxlint@^1.74.0` resolves to 1.75.0 whose peer range `oxlint-tsgolint >=7.0.2001` is unsatisfiable (max published is 0.x), so a fresh `npm install` ERESOLVE-fails. Hits the 4 NestJS services that pin it + `npm install` in their Dockerfile with NO lockfile (`media-service`, `social-service`, `core-service`, `chat-delivery-service`). Surfaced as a `CD - Deploy to Production` failure on the media-service image build; **production is unaffected** (CD only rebuilds CHANGED services, so these kept their prior images). Fix options: pin `~1.74.0` across the 4 services (fights dependabot), add npm `overrides` on oxlint's peer, or strip lint devDeps from the prod image. Own WP - not done.
