...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Data-driven re-editable PDF of the assos poster (bubbles = color+logo+president photo+roster, grouped by category) via the snapdom pipeline. Access `GlobalAdminOrBdeSuperAdminGuard`; project = persisted JSON layout (content re-resolved at render). **P0 DONE (38cf58b4 backend, 949f9cb6 frontend):** `AssociationCategory` table (migration `021`, seeds 5 sections) + `Association.categoryId` + `PosterProject` (jsonb `layout`, migration `022`) + `PosterService` CRUD; controllers `associations/categories` + `associations/poster` registered BEFORE `AssociationsController` (literal beats `:id`); typed API client, `/admin/carte` hub, dashboard card + nav, `categoryId` picker in `EditProfileTab`. **P1 DONE (this commit):** `frontend/src/lib/carte/{theme,generator,export}.ts` + `components/carte/PosterCanvas.svelte` + rewritten `/admin/carte/[id]/+page.svelte`. `buildPosterModel` = filter regular non-archived assos, resolve color/logo/president (role contains "presid"), group by category sortOrder (uncategorized last). PosterCanvas = fixed-1600px DOM that is BOTH preview AND snapdom target (initials-behind fallback for broken imgs, tilted polaroids, themed directory); editor scales it via `transform:scale` + ResizeObserver. `exportPosterPdf` = rasterize + jsPDF landscape/paginate. i18n `carte_poster_subtitle`/`carte_empty`/`carte_directory_heading`/`carte_zone_uncategorized`/`carte_settings_heading`/`carte_generated_note`/`carte_theme_*`/`carte_background_*`/`carte_scrim_label`/`carte_save_*`/`carte_export_*` (removed `carte_editor_coming_soon`). GOTCHAS: prod applies `.sql` migrations manually (each schema change needs entity AND numbered idempotent `.sql`); frontend `format` MUST pass `-c ../oxfmt.json` (bare oxfmt corrupts to double quotes). **BROWSER-VERIFY (snapdom can't run on host):** open a poster project -> each theme renders, presidents/logos resolve, bg upload + scrim, Export PDF matches preview. **NEXT = P2:** drag/resize/z-index editor + per-bubble property panel + persist `bubbles[]`.
