...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Data-driven re-editable PDF of the assos poster via snapdom; access `GlobalAdminOrBdeSuperAdminGuard`; project = persisted JSON `layout` blob (opaque, no migration; content re-resolved from live data at render). **BASE (P0-P3, shipped, git-recoverable):** category table (migrations `021`/`022`) + CRUD + `/admin/carte` hub; `lib/carte/{theme,generator,export,layout}.ts` + `PosterCanvas.svelte` freeform editor (absolute draggable stage, 4-corner center-fixed uniform resize, z-index, snap guides, per-bubble panel, free text). GOTCHAS: prod applies `.sql` migrations manually; frontend `format` already bakes `-c ../oxfmt.json` (do NOT re-pass it - duplicate `..` flag errors). **REFONTE (user feedback 2026-07-17), 3 steps, all local on main / NOT pushed. Full spec in the wiki's "Refonte" block.** **Step1 (0a468ded cleanup+UX; pleine-page correction 611ca9df) DONE:** one fixed `CARTE_STYLE` (dropped 3 theme presets; bg image replaces it), removed "N assos" subtitle, removed doodle+blob kinds (DELETED `doodles.ts`+`blobs.ts`, `Decoration`=`TextDecoration` only), FIXED drag "drops-on-next-click" (native img drag off + `preventDefault`/`setPointerCapture`), and editor "pleine page" = in-app overlay (`isFullPage` `$state` -> `fixed inset-0 z-50 overflow-auto bg-cn-bg p-5` on the editor grid; Fullscreen API dropped; i18n `carte_fullscreen_*`->`carte_fullpage_*`). **Step2 (agenda export) DONE (611ca9df):** `calendarExport.ts` container pinned to exact A4 ratio (`CALENDAR_CONTAINER_HEIGHT = round(210*1080/297)` = 764, explicit `height` on export container + `buildPreviewInnerHtml` wrapper, `MAX_CELL_H` clamp REMOVED so `CELL_H` divides full height), export on STANDARD `format:'a4'` landscape filled whole (`addImage(...,0,0,pageW,pageH)`) - no distortion, no white bar, prints borderless. Carte export still folded into Step3 (needs the A2 frame; `carte/export.ts` untouched at committed version). BROWSER-VERIFY (snapdom can't run on host): pleine-page toggles the overlay (chrome stays); export a sparse AND a busy month agenda -> single standard A4, no white bar, no crop, no distortion, bg to every edge. **Step3 (A2 frame + visual redesign) = NEXT:** poster stage becomes a FIXED A2 landscape frame (`STAGE_WIDTH` 1600 -> `STAGE_HEIGHT = round(1600/Math.SQRT2)` = 1131, `overflow:hidden`, bg covers whole frame); directory -> right-hand column listing ALL members grouped by asso; bubbles constrained to the left; each asso = large shape-configurable blob (re-create a border-radius shape catalog + per-asso shape selector) with president inside + bureau (asso admins) auto-radial polaroids around it; carte export then STANDARD A2 landscape filled whole (single `addImage`, content is A2-ratio). Decisions locked: bureau=auto-radial, annuaire=all members by asso, keep free text. BROWSER-VERIFY (snapdom can't run on host): Step1 pleine-page toggles + drag drops immediately + no theme picker/subtitle + doodles/blobs gone + text works; Step2 export a sparse AND a busy month agenda -> single standard A4, no white bar, no crop, no distortion, bg to bottom.
