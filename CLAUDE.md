...
- **Default to Paraglide for ANY new user-visible string, on the first draft.** Nothing types a string as user-visible, so no compiler enforces it. `bun run test` now compiles Paraglide itself (a build leaves it resolving to English); a rule that says "remember to run X first" is a missing dependency, not a rule.
...
- **RECONCILIATION is the only way this campaign's loss class can be SEEN** (`recon.mjs`). No per-check verdict substitutes for it. **It now covers the PHONE too** (2026-08-15): the native store is read in place over `plugin:sql|select` from CDP, because `adb pull` would put a real account's conversations on this machine and there is no on-device `sqlite3`. Last run, after the whole `25376b86` ladder: **RECONCILED both pairs, 9 shared conversations, 5 571 messages compared id by id, 0 differing, 0 one-sided, 0 notable in logcat** - the DM alone is 5 474 and matches to the row. **The phone needs `--rightUrl tauri.localhost`** (its WebView serves from there, not the domain); that is now the second line of `recon.mjs`'s usage, having cost a run by living eighty lines lower.
