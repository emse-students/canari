...
* \[x\] **Chat entry UX - instant, bottom-pinned, no skeleton flash (f438eb5a):** opening a discussion showed skeleton -> top-of-history -> teleport-to-bottom with a long gap. Root cause: login restore (`loadExistingConversations` phase 2, `conversations.ts`) already hydrates each conversation's latest local page into the map, but `selectConversation` -> `loadHistoryForConversation` flipped `isLoadingHistory=true` and `ChatArea` HARD-hid the list behind the skeleton for the whole network probe/replay (even the fast path does `fetchHistory(id,cursor,1)` first); then the fresh list painted at `scrollTop 0` before an async rAF pinned it down. Fix A: `showSkeleton = isLoadingHistory && visibleMessageGroups.length===0` - cached messages render instantly, network refresh updates in place, skeleton only on a cold/empty-local conversation. Fix B: new `entering` gate holds the scroll container at `opacity-0` from conversation-switch until the first pin lands (layout still computes -> scroll math valid), then fades in 150ms; `scrollToBottomSettled(onPinned)` fires `onPinned` on the frame it reaches bottom, clearing `entering`. All in `ChatArea.svelte`, no pipeline change. Verify (web+mobile): open a conversation -> appears instantly already at the latest message, no skeleton over cached msgs, no top->bottom jump; a brand-new/never-loaded conversation still shows the skeleton.

* \[~\] **Carte de la Vie Asso - editable poster generator (design doc `docs/wiki/carte-vie-asso.md`; ALL browser-verify pending - snapdom/jsPDF can't run on host).** Data-driven re-editable poster; access `GlobalAdminOrBdeSuperAdminGuard`; project = opaque persisted JSON `layout` blob (content re-resolved from live data at render). Files: `lib/carte/{theme,generator,export,layout,shapes}.ts` + `components/carte/PosterCanvas.svelte` + `components/ui/ColorPicker.svelte` + `/admin/carte/[id]/+page.svelte`. GOTCHAS: prod applies `.sql` migrations manually; `format` already bakes `-c ../oxfmt.json` (don't re-pass); `{@const}` must be an immediate child of a block (put logo-dims consts at the `{#if data}` top, not inside the unit `<div>`). **Shipped (all on main):** Base P0-P3 + Refonte Steps 1-3 + feedback pass 1 `fix carto` (1b32a339: migration 021 no longer seeds categories - fully admin-managed; `isolation:isolate` on stage fixes send-to-back-behind-bg; A2 frame `STAGE_HEIGHT=round(W/SQRT2)=1131`, `DIRECTORY_WIDTH=500`; `fitSeedScale()` fits the whole roster in-frame = "missing assos" fix; directory `$effect` auto-shrinks font to fit) + feedback pass 2: WP1 `60f38a6` (shared `ColorPicker` with new optional `onChange` prop used for bubble/deco color; editable persisted `titleColor`; canvas zoom toolbar 0.4-4x over a scroll wrapper sized to the scaled poster; drag no longer clamps to margins/directory - reaches+overflows edges, KEEP=40px; full-page toggle moved into Reglages; **debounced autosave 4s** armed after hydration, reads only content fields) + WP2 `0835b343` (blob relayered: plain color silhouette back, hero logo its own frame shape centered + allowed to overflow, bureau member-cards over the top arc in front, **president always a card** photo+name+role at blob bottom, name below; bureau+president share a `memberCard` snippet with full name + role; **dedicated `LOGO_SHAPES`** round/squircle/wide/tall via `PositionedBubble.logoShape` + picker; directory shows roles in parens). Current geometry: `CARD_WIDTH=400 CARD_HEIGHT=430 BLOB_SIZE=200 BLOB_CY=168 LOGO_BASE=112 RING_RADIUS=124 CARD_W=78 PRES_CARD_W=104`, top-arc `-PI+((i+0.5)/n)*PI`.
  * WP3 `377ab61b` = **searchable A0 PDF export (user chose the HYBRID raster+text approach).** New shared `lib/pdf/searchableRaster.ts::exportSearchablePdf(el, opts)`: measures every `[data-pdf-text]` box from the live DOM (reproduces on-screen wrapping + directory auto-shrink for free), sets those runs transparent, rasterises the visuals via the shared snapdom helper as a full-bleed A0 background, then re-draws the text as REAL vector text (jsPDF, Helvetica) at the measured boxes -> crisp + selectable/Ctrl-F over a pixel-faithful background, signature unchanged (still takes the DOM node). `carte/export.ts` rewired to it (`format:'a0'` landscape). PosterCanvas text nodes tagged `data-pdf-text` (title / card name+role / asso name / directory heading+asso+members / free-text-only-when-non-empty). **Real fonts embedded (7e41de1):** `lib/pdf/appFonts.ts` embeds static TTF (Nunito 400/700/800 + Fredoka 400/700) from `@expo-google-fonts/{fredoka,nunito}` (jsPDF can't use the woff2 *variable* families; static ttf imported as Vite asset URLs, base64'd at export only); `registerAppFonts(pdf)` + `pickAppFont(family,weight)` (Nunito extra-bold under own jsPDF name; helvetica fallback); overlay now matches the web typography. FOLLOW-UPS: (a) apply the same helper to `calendarExport.ts` (agenda) once the carte export is verified good - user wants it factored/reused; (b) directory members `<p>` split across the 2 columns could misplace its overlay box (edge case). CANNOT be verified on this host (jsPDF/snapdom are browser-only) - user must check: export -> A0 landscape, text crisp + Ctrl-F works, visuals match preview.

* \[x\] **MLS stuck-in-SYNC group fix - externalJoin-first Welcome recovery, hoisted out of the lock (7dd116c3):** diagnosed a 4-person group ("Les ROOTz", `8ab64b08`) wedged in SYNC forever while DMs healed fine. Prod DB (via `ssh canari` -> `docker exec infrastructure-postgres-1 psql -U canari -d auth_db`; SINGLE db `auth_db`, tables `dm_groups`/`dm_group_members`/`dm_device_group_memberships`/`mls_group_info`) confirmed the SERVER was healthy: GroupInfo present at `baseEpoch==activeEpoch`, device membership `active`. Root cause was purely client recovery ordering: a redelivered Welcome fails `NoMatchingKeyPackage` (our published KeyPackage orphaned from its local private key), the OLD code only did republish+welcome_request and escalated to the peer-independent `externalJoin` (self-rejoin via GroupInfo) only after 3 MORE failed Welcomes - which never arrive once the inviting peer's `handleWelcomeRequest` anti-livelock (`MAX_READD_ATTEMPTS=3`) suspends re-adds. RULED OUT (red herrings): (C) worker-keystore split is HANDLED - `mlsKeyPackage.worker` generates KPs in a separate WASM client but `generateKeyPackage` swaps the state back (`reloadClientFromState`, staleness guard) + `reconcilePublishedKeyPackages` purges orphans on reconnect; the "clean state" log is the READ-ONLY `mlsCrypto.worker` history-decrypt sandbox. (B) the Welcome IS ACKed/deleted (handler returns true); not redelivered from the same row. Fix: `setupMessageHandler.ts::handleWelcome` now drives `requestReAdd` (externalJoin-first, welcome_request fallback) IMMEDIATELY on failure #1, and ALL recovery is hoisted OUT of `runUnderMlsLock` (the lock is NON-reentrant and externalJoin/republishKeyMaterial re-acquire it -> a latent deadlock existed on the GroupAlreadyExists path when GroupInfo present). Locked callback now returns a `DeferredRecovery`, run after release. DM-vs-group asymmetry (user asked): same MLS join; groups fragile b/c (a) DMs are deterministically keyed `userA::userB` + recreatable/merge-dedup, named groups must rejoin the exact id; (b) more devices = more KP consumption + epoch churn; (c) diffuse re-add responsibility + the peer suspension. **VERIFY on device: deploy + reload -> stuck group self-heals via externalJoin (log `[MLS] externalJoin succeeded`); name appears.** GOTCHA: `runUnderMlsLock` is non-reentrant ("never re-acquire while holding") - any recovery seam (externalJoin/republish) MUST run outside it. `ssh` works from PowerShell tool ONLY (Bash tool's cloudflared ProxyCommand path is mangled).
