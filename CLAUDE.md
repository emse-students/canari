...
6. **FORM PRICING MATRIX - PLANNED IN FULL, NOTHING BLOCKED** (asked 2026-08-23, while the forms
    admin UI was being rebuilt; five decisions taken the same day). Prices discriminated by promo,
    formation, cotisation tier AND the answer to a question, through "Filtrer par..." checkboxes.
    Design in [`plans/form-pricing-grid.md`](plans/form-pricing-grid.md), queue substance in
    [backlog](docs/wiki/backlog.md), neither restated here. **The user's own correction is the
    design: a MATRIX, not an ordered rule list** - so no priority rule exists to get wrong, and
    completeness is a save-time invariant. Phase 1 is behaviour-free with no matrix, so it ships
    before any UI. The same bucket predicate also gates a question's visibility and who may submit.
