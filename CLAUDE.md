...
* \[x\] **WP-iOS-11 (CD) - GREEN end-to-end (run 29857076911, 2026-07-21):** `ios-release.yml` builds a signed `.ipa` on a free public-repo macOS runner and attaches it to the GitHub Release `vX.Y.Z` (on the real `workflow_run` trigger; the Upload-to-Release step is correctly SKIPPED on manual `workflow_dispatch`). Verified: `** BUILD SUCCEEDED **` -> `** EXPORT SUCCEEDED **` -> `build/ipa/Canari.ipa` (~12 MB artifact `ios-release`). The five fixes that got it there (all shipped): (1) NSE `Info.plist` removed from the appex Copy-Bundle-Resources phase (`313aa2a8`, "Multiple commands produce Info.plist"); (2) prebuild `libapp.a` step - cross-compile `mines_app_lib` staticlib to `Externals/arm64/release/libapp.a` because the pbxproj "Build Rust Code" phase is commented out + `Externals/` gitignored (`7a1aab6f`); (3) `rustup target add aarch64-apple-ios` before that cargo build (rust-toolchain.toml pins 1.93.0 with no `targets`, `3ce0a0b3`); (4) link `BackgroundTasks.framework` in the app target (WP-iOS-4 `BGTaskScheduler` was undefined, `b1a3fa87`); (5) split archive/export - Tauri's auto ExportOptions can't map two manual profiles, so `tauri ios build` runs tolerantly to make the signed `.xcarchive` and a separate step re-exports it with our own ExportOptions.plist (manual, `fr.emse.canari->Canari` + `fr.emse.canari.notifications->CanariNotifications`, `a0610d6f`). GOTCHAS for future edits: benign `ld: warning ... built for newer iOS version (26.2)` (runner SDK newer than deploy target - harmless); benign `CFBundleShortVersionString of app extension ('1.0') must match parent ('0.9.19')` (NSE Info.plist version is static - cosmetic, does NOT block; fix later by syncing the appex version if App Store validation ever rejects it). The profiles expire 2027-07-11 (manual `gh secret set` renewal, or the deferred fastlane automation). NEXT for a REAL release: cut a version bump so "Bump version on release" fires -> ios-release runs on `workflow_run` -> `.ipa` attaches to the release automatically.
  Durable facts: NO TestFlight (ship the `.ipa` as a release asset like Android's `.aab`/`.apk`); manual signing references two NAMED profiles `Canari` (app) + `CanariNotifications` (NSE) that must match the pbxproj `PROVISIONING_PROFILE_SPECIFIER` exactly, both installed from secrets `APPLE_PROVISIONING_PROFILE`/`APPLE_PROVISIONING_PROFILE_NSE`; team = "Les Rootz" `4CLNB8SR6L`. STILL OPEN: WP-iOS-10 `.lproj` strings are NOT yet referenced in the pbxproj (no localized variant group) - permission prompts stay FR-only until a Mac wires them. Guide: `docs/wiki/infrastructure/ios-ci-cd-setup.md`.
