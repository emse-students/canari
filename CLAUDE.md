...
* \[x\] **WP-COT-1 (data model + tag derivation, migration 025_cotisation_tiers.sql):** `association_products` gained `variantKey VARCHAR(100) NULL` + `variantLevel INT NULL` (both entity + DTO, NULL = single-tier asso, behavior unchanged). `deriveCotisationTag(slug, mode, now?, variant?)` appends `-${variant}` before the academic-year suffix when set; all 6 existing call sites still pass 2 args, fully back-compat. Tests in `cotisation-tag.util.spec.ts` (7 cases: lifetime/dated x with/without variant, back-compat equivalence).
