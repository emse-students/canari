...
**B. Parent-association Stripe delegation (feature, DONE - NOT PUSHED).** A club association routes its online payments to a PARENT association's Stripe Connect account and grants the parent read access to its accounting. "association" wording kept in UI (never "club").

Locked model: dedicated fields `paymentParentAssociationId` + `paymentDelegationStatus` (`pending`|`approved`|null), distinct from lists-only `parentAssociationId`. Parent MUST approve. Explicit = when approved ALL club payments route to parent even if club has own account. Accounting access = child purchase records + form payments + xlsx (NOT Stripe balance/dashboard). Reuse MANAGE_PRODUCTS. One level only (no chains; a parent can't itself delegate; a requester already-a-parent can't delegate). Single decision point = pure `resolvePaymentTarget()` in payment-delegation.util.ts (fails closed).

* \[x\] Phase 1 backend (41fdff8f): migration 017; entity fields; util; associations.service lifecycle + rerouted stripe getters; products.service checkout/charge/isActive gates; posts paid-post gate. Tests: util(6)+delegation(14)+products routing(3).
* \[x\] Phase 2 backend (208a0a5a): `exportAssociationPurchases` xlsx; `assertIsApprovedParentOf`; controller `sendXlsxDownload` helper; own + parent-scoped child purchases/export endpoints. Endpoints (all MANAGE_PRODUCTS): GET/POST/DELETE `:id/payment-delegation`; GET `:id/payment-delegation/children`; POST `.../children/:childId/{approve,reject}`; GET `.../children/:childId/purchases[/export]`; GET `:id/purchases/export`.
* \[x\] Phase 3 frontend + i18n + wiki (31df2bf5): api.ts client fns + shared `downloadXlsxFromApi`; `EditDelegationTab.svelte` (club-side request/status/cancel + parent-side approve/reject/revoke + expandable child accounting/export); Delegation tab in edit/+page.svelte (gated canManageProducts); `asso_deleg_*` keys fr+en; payments.md wiki section + associations.md tab row. Frontend gate green (check 0/lint/format via pre-commit); social-service jest(43) green. NOTE: full `make run-ci` not run (only touched surfaces gated).
