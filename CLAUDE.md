...
- DOCUMENTATION: technical docs in `docs/wiki/` (English, LLM-oriented, **search it before reading source**). User guides in `docs/user-guide/` (French). Root: `README.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `SECURITY.md`. Delete unused code immediately.
...
- CHANGELOG: one **SHORT** entry per change, as ITS OWN FILE in `changelog.d/` - never under `[Unreleased]`, which GitHub cannot merge in parallel ([README](changelog.d/README.md)). A sentence or two and a LINK (user, 2026-09-17). Reasoning, measurements and what was refuted go on the wiki page it points at. A long entry is not a thorough one, it is one nobody reads - and that records nothing.
...
1. **PHASE 1 IS FULLY DONE, ALL THREE ESTATES; PHASE 2 (`canari-emse.fr` -> `canari.emse.fr` ONLY, cercle/miconnect renames excluded) IS DONE TOO, 2026-09-24** - the app-side config PR (#1078) and the one-line vhost fix (no DSI ticket was ever needed) both merged/live; `canari.emse.fr` now serves Canari, verified. **NEW DURABLE RULE FROM THIS WORK**: a browser's IndexedDB/cookies do not follow a host change the way a native app's filesystem state does - no redirect, server- or client-side, may ever send an existing session from the old host to the new one ([estate-migration](docs/wiki/infrastructure/estate-migration.md#a-browser-cannot-follow-a-redirect-and-keep-its-state---there-must-never-be-one)). OIDC issuer rename and the cercle/miconnect DSI ticket stay deferred, not in this scope.
2. **P1 - `canari.emse.fr` CANNOT CARRY A WEBSOCKET AT ALL** (found 2026-09-25, still live) - `canari.conf` never forwards `Upgrade`/`Connection`, so real-time chat, presence and calling are silently unreachable on the now-primary hostname. Fix is two lines, written and untaken: this session's own write classifier refused the live nginx edit four times running. **OWED TO THE USER**: either a permission grant or apply the two lines directly ([backlog](docs/wiki/backlog.md#p1---canariemsefr-cannot-carry-a-websocket-at-all---diagnosed-2026-09-25-still-not-fixed)).
3. **P1 - A MEMBER WHO COMES BACK TO A COMMUNITY NEVER GETS ITS PAST: EVERY SEED REQUEST GOES TO AN OFFLINE MEMBER** (prod, 2026-09-24) - the election ignores presence the server already knows; fix proposed, awaiting the go-ahead ([backlog](docs/wiki/backlog.md)).
...
5. **P1 - a PLACEHOLDER held a member's seat**; only a member's CLIENT can say whether a LEAF is
...
6. **P1 - HEAL-repair HEALS 3 TIMES IN 10, SAME ASK CADENCE EITHER WAY**; two causes REFUTED by
   A/B, not to be re-opened ([backlog](docs/wiki/backlog.md)). HEAL-REVOKE stays four clean `PASS`.
7. **P1 - A DAMAGED MLS STATE IS CALLED A PIN ROTATION** (CORRUPT-2 + CORRUPT-1). The typed errors
...
8. **P1 - THE TUNNEL RUN TOKEN IS READ BY ANY LOCAL USER ON BOTH PROD BOXES** - `600` closes
...
9. **THE MLS AUDIT: THE DUPLICATE HALF IS CLOSED, THE DEAD ENDS ARE WHAT IS LEFT** - counts
...
10. **THE DEPENDENCY CHAIN** (user: *"un projet qui peut 'vivre tout seul'"*) - ONE merge mechanism
...
11. **P2 - 1429 LEGACY COTISATION ROWS ARE LOADED ON BOTH ESTATES (v0.17.1) AND NOT ONE CLAIM HAS
...
12. **BLOCKED ON HARDWARE** ([table](docs/wiki/backlog.md#owed-a-verification-and-nothing-else),
   [procedures](docs/wiki/device-verification.md)). **A precondition is NOT ambient.**
13. **UX/RENDERING - OPEN: REAL HARDWARE (all of it Chrome with an override) AND THREE of the ELEVEN
...
14. **A MEMBER COULD NOT PUBLISH: THE STAGE WAS `poll`, FOUND IN THE EDGE LOG ALONE** - the publish
...
15. **CALLING IS HELD OFF - `CALLS_ENABLED = false`** (user, 2026-09-01); FIVE switches move in ONE
    commit at revival ([calls](docs/wiki/frontend/modules/calls.md)). Prod HAS TURN, never used.
16. **ONE NAMED STARTING POINT FOR EVERY PHASE, STEP AND STEP GROUP** (user, 2026-08-25). Contract
    and audit in [backlog](docs/wiki/backlog.md).
17. **P1 - A DEVICE ASKS FOR A WELCOME FOR EVER AND THE MEMBER ANSWERING RESETS THE HEALING ROW** -
    five halves fixed 2026-09-04, **ONE PROD MEASUREMENT OWED**; read with its sibling P2 ([backlog](docs/wiki/backlog.md)).
18. **TWELVE MESSAGES DROPPED, PERMANENT COMMIT-LOG HOLE AT EPOCH 121** - four defects shipped in
    `v0.15.0`'s ancestors; RESIDUE and the 13:10 arm open ([backlog](docs/wiki/backlog.md)).
19. **`dev.canari-emse.fr` IS THE PRE-RELEASE TARGET** ([dev-environment](docs/wiki/infrastructure/dev-environment.md), the only copy). ONE open: a dev
...
20. **FROM THE USER: FIRST CONTACT, THE THEME NO NOTIF ROW HAS EVER ASKED ABOUT.** The 2026-09-08 PROD P1 is FIXED and SHIPPED in `v0.18.18`, owing ONE hardware run nothing
...
21. **ONE COMMIT, FOUR NOTICES ON SCREEN (#752, PROD 2026-09-16) - THE FRAME CARRIES AN ID AND
...
**2.1(a) IS PASSED; CHECK R IS FIVE OF SIX ON `0.18.21`, AND THE INSPECTABLE STORE WEBVIEW IT FOUND
IS FIXED IN `0.18.22`** ([device-verification](docs/wiki/device-verification.md)); **WP-RESTORE-1** ACCEPTED.
