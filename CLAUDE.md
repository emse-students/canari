...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Recreates the hand-made assos poster (bubbles = asso color+logo+president photo+roster, grouped by theme) as a data-driven, re-editable PDF via the existing snapdom pipeline. Decisions ACTED: managed category TABLE (not enum) + 1 primary category/asso + per-bubble override in editor; access = `GlobalAdminOrBdeSuperAdminGuard` (global admin + BDE super-admin); project = persisted JSON layout (photos/members re-resolved at render -> always current). Phases: P0 foundations / P1 generator+static render+PDF / P2 drag-drop editor / P3 doodles+themes+free text. **P0 BACKEND DONE (this commit), social-service:** entity `AssociationCategory` (+ migration `021`, seeds 5 poster sections) + `Association.categoryId` loose uuid (nulled on category delete) exposed via list spread + `UpdateAssociationDto.categoryId` (blank->null); entity `PosterProject` (jsonb `layout`, + migration `022`) + `PosterService` CRUD. Two NEW controllers `AssociationCategoriesController` (`associations/categories`, GET public, writes guarded) + `PosterController` (`associations/poster`, all guarded) registered BEFORE `AssociationsController` in the module so literal routes win over `:id` (reuses nginx `/api/associations`, no infra change). GOTCHA: prod applies `.sql` migrations manually (`synchronize` only in dev); each schema change needs BOTH the entity AND a numbered idempotent `.sql`. **NEXT = frontend package:** API client (categories + poster) + `categoryId` on `Association` type + category manager + picker in asso settings + `/admin` carte route stub + dashboard card + FR/EN i18n. Then P1 render.
