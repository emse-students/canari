...
OPEN:
* **Store auto-publish (CI wired this session; awaiting user-provided secrets):** both upload paths are now coded and gated to SKIP-not-fail until their secrets exist, so builds stay green. **iOS** `ios-release.yml` runs `xcrun altool --upload-app` (App-Store-signed `.ipa` -> TestFlight; a raw `.ipa` is NOT sideloadable from a Release, unlike Android's `.apk`) - needs secrets `APP_STORE_CONNECT_API_KEY_P8` (base64 of the `.p8`), `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID` (App Store Connect API key, role App Manager, NO Mac) + the app record created in App Store Connect. **Android** `android-release.yml` `r0adkll/upload-google-play` publishes the AAB to the `internal` track, gated on a `Check Google Play secret` step (the `secrets` context is illegal in a step `if:`) - needs `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` + a ONE-TIME manual first AAB upload in the Play Console (Google refuses API uploads before that). Full user-facing runbook (FR, intentional exception): [docs/publication-stores.md](docs/publication-stores.md). User is procuring the keys; resume by verifying `gh secret list` then triggering a test Release.
