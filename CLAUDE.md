...
#### MULTI-TIER COTISATIONS ROADMAP (Cercle + generalization)

CONTEXT: some assos have >1 cotisant type (Le Cercle: forfait alcool XOR sans-alcool, upgrade = pay the delta). Generalized the cotisation model from "one asso = one implicit cotisant tag" to "N named tiers + gating on ANY tag". `userId` Canari IS the Authentik/MiConnect `sub` (`findOrCreateFromOidc` uses `userinfo.sub` as the PK), so no id-mapping table is needed anywhere in the Cercle integration.

WP-COT-1..6 shipped (migrations 025-027, durable gotchas only):
* `association_products` gained `variantKey`/`variantLevel` (NULL = single-tier, back-compat); `deriveCotisationTag(slug, mode, now?, variant?)` appends `-${variant}` before the academic-year suffix.
* `memberPriceTag` - when set, `amountCentsMember` applies iff the buyer holds THAT specific tag (no fallback to the generic asso-wide check). Fulfillment transaction-wraps the grant + `revokeSiblingTierTags` (XOR switch between tiers).
* `requiredTags` (`text[]`, OR semantics) generalizes gating beyond `membersOnly` - not scoped to the owning association, takes precedence when set. `isBuyerCotisant`/`membersOnly` now enumerate ALL of an asso's tier `variantKey`s (via `tierVariantKeys()`) rather than just the base tag, so sibling-tier cotisants of a multi-tier asso are correctly admitted.
* Inbound `GET /api/public/cotisant-status?assoSlug=&sub=` (`PublicController`, social-service) gated on `X-Api-Key` vs `CERCLE_API_KEY`, throttled 20 req/min. Outbound `dispatchCercleWebhook` (HMAC-SHA256, 3 retries) payload is `{ productId, userId, amountCents, paymentIntentId, timestamp }` - `userId` doubles as `sub`, `paymentIntentId` is the idempotency key. Both directions documented in `docs/wiki/cotisations.md` ("Cercle integration").
* Remaining manual step: set the real `webhookUrl`/`webhookSecret` on the production `balance_topup` product via `/admin/cercle` once Cercle provides them (deferred, no credentials yet as of 2026-07-22).
* **WP-COT-6 (admin tab, multi-tier):** `ProductsService.create` now auto-derives `grantedTagName`/`tagExpiresAt` server-side for any `type: 'membership'` product from `variantKey` (client value ignored, requires `cotisationMode` set); `provisionCotisationProduct` resyncs ALL membership products' tags on config edits, not just one. `EditCotisationsTab.svelte` lists every tier (base always first, undeletable), lets admins add tiers (`name`/price/`variantKey`, locked after creation) and set `memberPriceTag`+`amountCentsMember` via a sibling-tier dropdown. Roster + `.xlsx` export gain a "Forfait" column/badge via `UserTagService.buildTierLabelMap` (base tier left unlabeled).
* **WP-COT-7 (shop tiers + delta price):** `/shop` (`routes/shop/+page.svelte`) sorts each association's products base-tier-first (`compareTiers`, stable). Member pricing now gates on real eligibility (`qualifiesForMemberPrice`) instead of showing `amountCentsMember` unconditionally: no `memberPriceTag` -> `viewerIsCotisant`; tier-upgrade product -> find the sibling whose `grantedTagName === memberPriceTag`, eligible iff `viewerActiveTier === sibling.variantKey` (client-side join, mirrors `resolvePurchase`'s server check, no tag re-derivation). Current tier gets a badge. `pricingTagName` (forms) verified generic already - `searchTagCatalog` seeds the autocomplete from every product's `grantedTagName` including tier variants, so no backend change was needed.
* \[ \] **WP-COT-8 (P3, optional inclusion):** honour `variantLevel` so a gate "requires tier >= N" is satisfied by any higher tier (NOT needed for the Cercle - recharge is generic, upgrade is explicit `memberPriceTag` - so ship only if a real need appears). Update [docs/wiki/cotisations.md](docs/wiki/cotisations.md) (tiers + both API directions) and the end-user guide.
