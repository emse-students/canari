...
8. **UX/RENDERING: TWO DECISIONS TAKEN AND UNSTARTED, AND ONE SHAPE THE USER STILL OWES.** *"Poser la reference ET tout aligner maintenant"* (2026-09-09): two declared icon-button sizes (28px on a message hover strip, 38px everywhere else) and **all 187 call sites swept in the same change**, gate included - **do not re-litigate it, start with it**. Beside it, a GRID page is capped at the FEED's reading width - `/associations` draws 2 columns in 680px, a month gets 95px a day, 22 routes of 26 - and **which shape replaces it is the user's call**, Google Agenda being full-bleed where Facebook keeps a narrow feed and spends the rest on an aside. Both, and the only count, are in [backlog](docs/wiki/backlog.md).
