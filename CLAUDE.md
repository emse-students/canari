...
**B. Parent-association Stripe delegation (feature, IN PROGRESS).** A club association routes its online payments to a PARENT association's Stripe Connect account and grants the parent read access to its accounting. Keep "association" wording in UI (never "club").

Locked scope (decisions 2026-07-14): dedicated fields `paymentParentAssociationId` + `paymentDelegationStatus` (`pending`|`approved`|null), distinct from the lists-only `parentAssociationId`. Parent MUST approve. Explicit toggle = when approved, ALL club payments route to parent even if the club has its own account. Accounting access = child's purchase records + form payments + xlsx export (NOT Stripe balance/dashboard). Reuse MANAGE_PRODUCTS (no new flag). One level only (no chains; a parent can't itself delegate; a requester that is already a parent can't delegate).

* \[x\] Phase 1 backend (commit 41fdff8f): migration 017_payment_delegation.sql; entity fields; payment-delegation.util.ts (pure `resolvePaymentTarget`/`isDelegating`, fails closed); associations.service `resolvePaymentTarget` + rerouted getStripeAccountId/isStripePaymentsReady/assertStripePaymentsReady + lifecycle (requestPaymentDelegation/cancelPaymentDelegation/listDelegatedChildren/approvePaymentDelegation/rejectPaymentDelegation); products.service checkout/charge/isActive gates via `resolvePaymentTargetFor`; posts.controller paid-post gate; controller endpoints (see Phase 3). Tests: payment-delegation.util.spec (6) + associations.service.delegation.spec (11) + products.service.spec delegated routing (3).
* \[x\] Phase 2 backend (commit 208a0a5a): products.service `exportAssociationPurchases` (xlsx); associations.service `assertIsApprovedParentOf`; controller `sendXlsxDownload` helper (cotisants export folded in); endpoints `GET :id/purchases/export`, parent-scoped `GET :id/payment-delegation/children/:childId/purchases[/export]`. Tests: assertIsApprovedParentOf (3). social-service tsc+eslint+jest(43) green.
* \[ \] Phase 3 frontend + i18n + wiki (NOT STARTED). Backend endpoints ready (all MANAGE_PRODUCTS): `GET/POST/DELETE :id/payment-delegation` (state / request {parentAssociationId} / cancel), `GET :id/payment-delegation/children`, `POST :id/payment-delegation/children/:childId/{approve,reject}`, `GET :id/payment-delegation/children/:childId/purchases[/export]`, `GET :id/purchases/export`. Plan: (1) API client fns in frontend/src/lib/associations/api.ts (getPaymentDelegation, requestPaymentDelegation, cancelPaymentDelegation, listDelegatedChildren, approve/reject, listChildPurchases; export = window.open/download URL like existing xlsx). (2) New `EditDelegationTab.svelte` under components/associations/edit/ with BOTH club-side (select parent from association list, request, show status pending/approved + parentReady, cancel) and parent-side (approval queue from listDelegatedChildren, approve/reject, expandable child accounting reusing the EditAchatsTab table + export button). (3) Wire into src/routes/associations/[slug]/edit/+page.svelte: add tab to `editSection` union + tab button + render block, gate on `canManageProducts`; tabs currently profile/members/documents/achats/cotisations/payments/formulaires/danger. (4) i18n: add keys to BOTH messages/fr.json + en.json (all cotisation/achats keys are prefixed `asso_achats_*`, `asso_*`). (5) Wiki: docs/wiki/payments.md (+ cross-link associations/admin wiki) describing delegation model. Gotcha: `/products/all` viewerIsCotisant precedent - keep server-authoritative. Frontend gate before commit: bun run check/lint/format (pre-commit hook sweeps whole frontend + re-stages).
