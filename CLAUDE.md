...
* **H. Normalization sweep (ongoing).** Paraglide FR/EN + English comments + wiki across remaining chat/community modules. NORMALIZATION-PLAN.md is [x] everywhere except REUSE1; H tracks residuals from post-plan feature work. DONE (2a9e57b9 + 73be1f0a): user-visible FR in chat/channels -> Paraglide (historySystemEvents replay now reuses live-path keys incl. new chat_system_member_left; messaging send/deleted errors + reply preview; pinChange thrown errors -> new profile_pin_error_*), + full EN normalization of those 3 .ts files. REMAINING residuals (accent-grep hits, fold-in as touched): FR `<!-- -->` comments in components/chat/{ChatArea,ChatGroupPanel,ChatHeader,ConversationTile,DeviceManagementPanel}.svelte; FR dev-logs/comments in utils/chat/{conversations,fcmCache,history,groupSyncEligibility,outbox,sharedContent}.ts; plus VoiceRecorder.svelte:77 + AssociationDocumentManager.svelte:224 (non-chat, `Erreur ...` console). Note: systemMessageHandler.ts:115 has a FR `Echec traitement distribution` log (mls-client, not utils/chat).
