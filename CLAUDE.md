...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Data-driven re-editable PDF of the assos poster via the snapdom pipeline; access `GlobalAdminOrBdeSuperAdminGuard`; project = persisted JSON `layout` blob (content re-resolved from live data at render). **P0 DONE (38cf58b4 backend, 949f9cb6 frontend):** category table (migration `021`) + `Association.categoryId` + `PosterProject` jsonb layout (migration `022`) + CRUD; `/admin/carte` hub + category picker. **P1 DONE (7500e4f3):** `lib/carte/{theme,generator,export}.ts` + `PosterCanvas.svelte` static render + `exportPosterPdf` (rasterize + jsPDF). **P2 DONE (this commit):** freeform editor. `lib/carte/layout.ts` = `PositionedBubble` (x/y/scale/z + colorOverride/showPresident) + `seedBubbleLayout` (auto-grid from zones) + `mergeBubbleLayout` (reconcile saved vs live: keep placed, add new, drop removed) + `stageHeight`. `PosterLayout` gained `bubbles?`+`directoryVisible?` (opaque blob, no migration). `PosterCanvas` rewritten to a draggable absolute stage: cards drag (poster delta = client delta / `viewScale`), 4 corner handles = uniform center-fixed resize, z-index, selection outline, click-empty deselect; directory is a toggleable footer. Editor page holds `positioned[]`+`selectedId`, per-bubble panel (color+reset / president toggle / front-back / reset pos), persists `bubbles`+`directoryVisible`, nulls selection before export (no handles in PDF). i18n `carte_directory_toggle`/`carte_editor_hint`/`carte_panel_*` (FR+EN). GOTCHAS: prod applies `.sql` migrations manually; frontend `format` MUST pass `-c ../oxfmt.json`. **BROWSER-VERIFY (snapdom can't run on host):** drag/resize/z-index behave, per-bubble color/president/reset persist across reload, directory toggle, Export PDF matches preview sans handles. **NEXT = P3:** doodle palette, free text, theme-independent background blobs, snap guides.
