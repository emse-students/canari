...
**Build-warnings audit (2026-07-23, COMPLETE - `08101c5a` + `48fdfe02`):** local `:app:compileUniversalReleaseKotlin` now compiles the whole Android module graph with ZERO warnings; backend oxlint reports zero across all 5 packages; frontend vite build + svelte-check + cargo check (src-tauri, mls-wasm) clean. Levers (durable): iOS ld noise killed via `IPHONEOS_DEPLOYMENT_TARGET=14.0` on the cargo step of `ios-release.yml`; appex/parent CFBundleVersion via `bump-app-version.sh` patching `canari_iOS/Info.plist` literals; KeystorePlugin API-30 guard (new keys only); `KeyboardMediaBridge` REWRITTEN on the receive-content pipeline (`ViewCompat.setOnReceiveContentListener` + view-based `createWrapper`, non-IME sources pass through; compat layer handles URI permission) - GIF keyboard flow needs a [device] re-verify; `generated/RustWebView.kt` dropped no-op WebSQL `databaseEnabled` + `generated/WryActivity.kt` targeted `@Suppress` (both marked CANARI CUSTOM PATCH, re-apply after regen); root `build.gradle.kts` silences cargo-registry modules only (Kotlin `suppressWarnings` + javac `-Xlint:-options`); `android.suppressUnsupportedOptionWarnings` in `gradle.properties` covers the two deliberate flags; `.oxlintrc.nest.json` turned OFF `no-misused-spread` (entity->DTO spread is the idiom). Post-v0.10.3 residuals fixed in `a6e2fa6a`: bump-version.yml now stages `canari_iOS/Info.plist` (appex/parent version warning) + NSE `LIBRARY_SEARCH_PATHS` arch-scoped (dropped phantom `Externals/x86_64/debug` ld warning) - both verify on the NEXT release. Remaining non-actionable: xcodebuild destination-pick + AppIntents metadata lines (xcodebuild's own stdout, no lever). Dependabot TS 7.0.2 PRs (#162/#168) fail CI on `ts-jest@29` peer conflict - blocked upstream until ts-jest supports TS7, not our breakage.
...
STORE PUBLISH (CI fully wired + all 4 secrets set; verified end-to-end on v0.9.21, v0.10.1 and **v0.10.3** - the WP-XP-4 + warnings-sweep release: iOS IPA -> TestFlight "UPLOAD SUCCEEDED", Android AAB -> Play production "Successfully committed", CD deploy green. v0.10.2 died at the Play publish step - superseded, versionCode unconsumed):
...
* \[x\] **WP-XP-1 Notification quick actions** (shipped + pushed): inline reply + mark-as-read from the shade, both platforms, app fully killed. Gotcha: `AppMessage` protos are built natively (no TS runtime) via shared Rust encoders (`proto_fields.rs`, exposed as JNI `nativeBuild*Proto` + iOS FFI `canari_native_build_*_proto`), then routed through the EXISTING outbox-drain path unchanged. The outbox mirror rewrite on both platforms must persist the `silent` flag or a control event loses it on retry and resends as a visible push (fixed on both, was silently dropped before). **Kotlin gotcha (v0.10.1 release build):** `OutboxMirrorEntry` was declared inside the `CanariFirebaseMessagingService` companion object - Kotlin lifts companion FUNCTIONS to the class name but NOT nested classes, so `CanariNotificationActionReceiver`'s `CanariFirebaseMessagingService.OutboxMirrorEntry` was an Unresolved reference and `:app:compileUniversalReleaseKotlin` failed. Push-CI runs NO Kotlin/Android compile (only bun/svelte-check) so it only blew up in the release build; fixed by moving the data class to the OUTER class body (`62d4f30d`). Any nested type shared with the receiver must live on the outer class, not the companion. Mark-read is cross-device-sync only, no local unread/readBy reconciliation. **iOS category gotcha:** the buttons only show when the notif carries `categoryIdentifier == "canari_message_category"`; the app-alive path (`CanariShowLocalNotification`) stamps it AND the NSE (`applyMessageContent`, killed-app-only path) must stamp it too - the backend sends no `aps.category` (was missing on the NSE side, fixed). Arch: `docs/wiki/services/chat-delivery.md` (Notification quick actions).
* \[x\] **WP-XP-2 App-icon unread badge** (shipped + pushed): launcher badge = distinct unread CONVERSATIONS (not messages), driven entirely off displayed message notifications (no separate counter). Android: `refreshBadgeSummary`/`countUnreadConversations` set the count via summary `setNumber()` (native only, NO ShortcutBadger; some old launchers show a dot). iOS has TWO writers by process state: app alive -> `CanariUpdateAppBadge` (`setBadgeCount` iOS16+ / `applicationIconBadgeNumber`); app killed -> NSE writes `content.badge` (`applyBadgeCount`, no UIApplication in an extension). Conversation key = per-conv `threadIdentifier` (NSE) or stable request id (flat `canari_messages` in-app); social/form gated out via thread/deep-link. Arch: `docs/wiki/services/chat-delivery.md` (App-icon unread badge).
* \[x\] **WP-XP-3 Rich media notifications** (shipped + released in v0.10.1): image/GIF messages show their decrypted thumbnail in the notification, both platforms, app fully killed. Scope = **images + GIF only** (video/audio keep text preview); MLS DM/group only (channels stay text). Cap 2 MB. Since media is E2E (server holds only opaque AES-256-GCM ciphertext, CEK/IV inside the MLS msg), the native builder fetches + decrypts the blob itself: (1) `extract_full_message_info` (`proto_fields.rs`) now emits `mediaId`+b64 `mediaKey`/`mediaIv`+`mimeType`; (2) killed app has no user JWT -> new PushSecret proxy `GET /api/mls/push/media/:mediaId` (chat-delivery, 2 MB cap -> 413) relays from media-service's new server-to-server `GET /api/media/internal/:id` (X-Internal-Secret); (3) new leaf FFI `nativeDecryptMedia` (JNI, returns bytes) / `canari_native_decrypt_media`+`canari_free_bytes` (C-ABI) reuse `background::decrypt_media_blob` (channel AES-GCM path); (4) Android attaches via `MessagingStyle.Message.setData(mime, FileProvider uri)` from `cacheDir/tauri/notif_media/` (keeps stacking+quick actions); iOS attaches `UNNotificationAttachment` and the **media thumbnail outranks the avatar** (iOS shows only the first image attachment) - `CanariFetchAndDecryptMedia` (foreground) + NSE `fetchAndDecryptMedia`/`attachImage` (killed). Gotcha: only `mediaKind=="image"` (covers GIF) rendered. chat-delivery needs `MEDIA_SERVICE_URL` (default `http://media-service:3011`) + `INTERNAL_SECRET` (already set). Arch: `docs/wiki/services/chat-delivery.md` (Rich media notifications).
* \[x\] **WP-XP-4 Boot/relaunch re-registration** (shipped): Android `CanariBootReceiver` (`BOOT_COMPLETED`+`MY_PACKAGE_REPLACED`, exported=false - protected broadcasts) force-reads the FCM token (`Tasks.await`), persists + re-registers it via `refreshTokenOnBackend` (moved to the service companion - function, not class, so no companion-lifting gotcha), then drains the outbox via the shared `drainOutboxBackground`. iOS side was ALREADY covered by WP-iOS-8's launch-time `tokenWithCompletion` force-fetch (no OS boot hook exists on iOS) - nothing added there. Manifest guard test extended (`androidFcmManifest.test.ts` now also asserts both receivers + the boot permission). Arch: `docs/wiki/services/chat-delivery.md` (Boot/relaunch re-registration).
