...
* \[ \] **WP-iOS-1 (P1, auth/deep-link) [device]:** OIDC on iOS. `auth.ts:116` redirect `fr.emse.canari://callback` is Android-gated -> add iOS (custom scheme + applinks already in entitlements/tauri.conf). `auth.ts:172` opens the system browser (Custom Tabs) Android-only -> iOS needs the equivalent (SFSafariViewController/opener) instead of navigating the WKWebView away. Flow-risk: verify login end-to-end on device.
* \[ \] **WP-iOS-2 (P1, resume) [device]:** foreground heartbeat. `ChatBackgroundService.svelte:705` starts `mls_foreground_heartbeat` on Android only. Decide: does iOS need the JS heartbeat, or does it rely solely on native `canari_ios_on_resume`/`on_pause` (`ios_ffi.rs`)? Confirm on device the warm engine and native bg engine hand off cleanly.
* \[ \] **WP-iOS-3 (P1, native):** `keystore_ok.flag` on iOS launch. Android writes it + migrates `pending_push_secret.txt`->Keystore in `CanariApplication`. Confirm the iOS launch path (`canari_ios.mm`/`canari_push.mm`) also writes `keystore_ok.flag` + runs migration, else `check_push_secret_health` reports `no_secret` -> false "keystore lost" banner on iOS.
* \[ \] **WP-iOS-4 (P1, native) [device]:** background-execution model. Info.plist declares `processing`/`fetch` UIBackgroundModes but NO `BGTaskScheduler` identifiers are registered (needs `BGTaskSchedulerPermittedIdentifiers` in Info.plist + handler wiring to `canari_native_cleanup_pending_db`). Android's `MlsBackgroundWorker` (WorkManager, expedited) has no iOS peer. Also: accept that iOS force-quit = no data-push delivery (structural, document it), and verify the backend sends `content-available:1` for silent frames.
