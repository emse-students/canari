...
OPEN:
* **App Store / TestFlight upload (deferred, decision pending):** the exported `.ipa` is App-Store-signed but a raw `.ipa` is NOT installable on iOS from a release asset (unlike Android's `.apk`) - distribution requires TestFlight/App Store. Automatable in CI with NO Mac via an App Store Connect API key (`.p8` + Issuer ID + Key ID as secrets) + `xcrun altool --upload-app`. Same key would also drive fastlane `match`/`sigh` to auto-renew the 2027-07-11 profiles + auto-publish Android (`r0adkll/upload-google-play`, `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` already a secret). Deferred 2026-07-21; user procuring the API key.
