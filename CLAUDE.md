...
* \[~\] **Carte de la Vie Asso - editable poster generator (ACTIVE, design doc `docs/wiki/carte-vie-asso.md`).** Data-driven re-editable PDF of the assos poster via the snapdom pipeline; access `GlobalAdminOrBdeSuperAdminGuard`; project = persisted JSON `layout` blob (content re-resolved from live data at render). **P0 (2fd438f5/8fff69ad) + P1 (10ef43b9) + P2 (57587dfb) DONE:** category table (migrations `021`/`022`) + CRUD + `/admin/carte` hub; `lib/carte/{theme,generator,export,layout}.ts` + `PosterCanvas.svelte` freeform editor (draggable absolute stage, 4-corner uniform center-fixed resize, z-index, per-bubble panel: color/president/front-back/reset pos); persists `bubbles`+`directoryVisible`; PDF export nulls selection. **P3a (ddb5696c) + P3b (0dae97ac) + P3c (dfadadec) DONE** (decoration layer riding one shared `Drag{target:'bubble'|'decoration',baseWidth}` machinery over `[data-el-root]` elements): P3a free-text (`Decoration` union + `TextDecoration`, Elements palette + decoration panel), P3b doodles (`DoodleDecoration` shape/color as inline lucide SVGs, `lib/carte/doodles.ts` 12-shape catalog), P3c snap guides (`collectGuides`/`nearestSnap` in `PosterCanvas.svelte`, dashed lines gated on `editable`, Alt = free placement; no data model/migration). **P3d (668412f8) DONE - closes P3:** third decoration kind `BlobDecoration`(shape/color/opacity) on the same machinery. New `lib/carte/blobs.ts` (`BLOB_SHAPES` 6 organic CSS `border-radius` silhouettes - self-contained/snapdom-safe, `blobRadius` first-shape fallback, `isBlobShape`); `layout.ts` adds `BLOB_BASE_SIZE`(280)/`createBlobDecoration`(opacity 50) + per-kind `sanitizeDecorations`/`stageHeight` branches; `PosterCanvas` `{:else if deco.kind==='blob'}` branch (inner div = organic radius + color + `opacity/100`); editor `addBlob` (drops behind everything via `min(z)-1`) + "Blobs" palette (preview swatches) + blob-only opacity slider gated on `selectedBlobDeco` narrowed derived (color/front/back/delete common). i18n `carte_blobs_label`+`carte_blob_*`. GOTCHAS: prod applies `.sql` migrations manually; frontend `format` MUST pass `-c ../oxfmt.json`; layout stays an opaque JSON blob (no migration). **REFONTE (user feedback 2026-07-17), 3 steps.** **Step1 (cleanup+UX) DONE, gates green, NOT pushed:** dropped the 3 theme presets -> `theme.ts` exports one fixed `CARTE_STYLE` (vitamine look; bg image replaces it); removed the "N assos" subtitle; removed doodle+blob decoration kinds (DELETED `doodles.ts`+`blobs.ts`, `Decoration`=`TextDecoration` only, free text kept); added **fullscreen** toggle (Fullscreen API, `editorEl` + `fullscreenchange` effect); FIXED the drag "drops on next click" bug (native img drag off: `draggable=false`+`-webkit-user-drag:none` on every img; `beginMove`/`beginResize` do `preventDefault()`+`setPointerCapture()`). `layout.theme` now optional/ignored. **NEXT = Step2 export:** ONE A2 landscape page, bg full-bleed to bottom (no white bar), same fix for the AGENDA export (`lib/utils/calendarExport.ts`). **Then Step3:** each asso = large shape-configurable blob (reuse a border-radius shape catalog for the asso blob) with president inside + bureau (asso admins) auto-arranged as polaroids AROUND it; directory -> right-hand column listing ALL members grouped by asso. Decisions locked: bureau=auto-radial, annuaire=all members by asso, keep free text, fix agenda too. BROWSER-VERIFY step1 (snapdom can't run on host): fullscreen toggles; drag drops immediately (no 2nd click); no theme picker/subtitle; doodles+blobs gone, text still works.
