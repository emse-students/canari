...
STORE PUBLISH (CI fully wired + all 4 secrets set this session):
...
* **Android** `android-release.yml` `r0adkll/upload-google-play` publishes the AAB to **`production`** (authoritative, uploads the bundle) AND best-effort to **`internal`** (`continue-on-error` - Google forbids re-uploading a versionCode, so the 2nd track assignment soft-skips; production already reaches everyone). Both gated on a `Check Google Play secret` step (the `secrets` context is ILLEGAL in a step `if:` - use an output step). Secret `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` set (`canari-ci@canari-496310.iam.gserviceaccount.com`). **Manual one-time still required:** create the app + first production release in the Play Console (Google refuses API uploads before the first manual release).
