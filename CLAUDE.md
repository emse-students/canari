...
* \[~\] **WP-iOS-11 (CD):** `ios-release.yml` now MIRRORS `android-release.yml` (trigger = `workflow_run` after *Bump version on release* + `workflow_dispatch`; no `force` gate; attaches the signed `.ipa` to the GitHub Release `vX.Y.Z`). NO TestFlight (user dropped it - ship the `.ipa` as a release asset like the Android `.aab`/`.apk`). Léon's commit `6cf14988` did the NSE Mac wiring (target `CanariNotifications`/`fr.emse.canari.notifications` added to pbxproj, embedded, manual signing) + posted all `APPLE_*`+`GOOGLE_SERVICE_INFO_PLIST` secrets (team = "Les Rootz" `4CLNB8SR6L`). BLOCKER before activation: the CI now needs a SECOND profile secret `APPLE_PROVISIONING_PROFILE_NSE` (manual signing references two NAMED profiles: `Canari` for the app + `CanariNotifications` for the NSE - names must match the pbxproj `PROVISIONING_PROFILE_SPECIFIER` exactly). Léon must (portal): create App ID `fr.emse.canari.notifications` + App Group `group.fr.emse.canari` on both App IDs, generate the two distribution profiles named exactly `Canari`/`CanariNotifications`, and `gh secret set APPLE_PROVISIONING_PROFILE_NSE`. FIRST CI run (29852146029) got EVERYTHING config-side green (secret validation, cert import, BOTH profiles installed by name, .env) and failed only in `xcodebuild` with `Multiple commands produce .../CanariNotifications.appex/Info.plist` - the NSE `Info.plist` was BOTH `INFOPLIST_FILE` and in the target's Copy Bundle Resources phase. FIXED in pbxproj (removed the `Info.plist in Resources` PBXBuildFile + its use in the NSE `PBXResourcesBuildPhase`) -> commit `313aa2a8`. SECOND run (29852852357) got past that and failed at `ld: library 'app' not found` linking BOTH the NSE and the app: the pbxproj "Build Rust Code" phase is COMMENTED OUT (`#bun tauri ios xcode-script...`, because Xcode's PATH lacks bun) and `Externals/`+`*.a` are now gitignored, so a clean CI checkout never produces `libapp.a` (Léon's local builds only work off a stale Externals lib). Crate lib name is `mines_app_lib` -> Tauri's convention copies `libmines_app_lib.a` to `Externals/<arch>/<config>/libapp.a`. FIX: added a "Prebuild Rust static lib (libapp.a)" workflow step (`cargo build --lib --release --target aarch64-apple-ios` then copy to `gen/apple/Externals/arm64/release/libapp.a`) before `tauri ios build`. AUTONOMOUS LOOP IN PROGRESS (user 2026-07-21): push -> `gh workflow run "iOS Release"` -> `gh run watch` -> read `ld`/xcodebuild errors from the log -> surgical fix -> repeat until the `.ipa` builds. Runs are free (public repo, macOS runner). The build reaches xcodebuild ~4-5 min in; a green build takes longer. THIRD run (29853886112): prebuild failed `error[E0463]: can't find crate for 'core'` compiling libc for aarch64-apple-ios - `rust-toolchain.toml` pins 1.93.0 with NO `targets`, overriding the workflow's `dtolnay@stable` (which had the iOS targets), so cargo's actual toolchain lacked iOS std. FIX (commit `3ce0a0b3`): added `rustup target add aarch64-apple-ios` before the cargo build in the prebuild step (dir override makes it land on 1.93.0). FOURTH run (29854598933): prebuild + NSE link now GREEN; the app target (`canari_iOS`) failed `Undefined symbols for architecture arm64: _OBJC_CLASS_$_BGTaskScheduler / _BGProcessingTaskRequest` referenced from `canari_push.o` - WP-iOS-4 uses `BGTaskScheduler` but the pbxproj never linked `BackgroundTasks.framework` (only UIKit/WebKit/Metal/Security/UserNotifications/etc). FIX (commit pending): added `BackgroundTasks.framework` (PBXBuildFile `BD49C8A0...`, PBXFileReference `D4E5F6...EF02`) to the app's PBXFrameworksBuildPhase `98A78BDE...` + Frameworks group. The prebuilt libapp.a triggers benign `ld: warning ... built for newer iOS version (26.2) than being linked (18.2/15.6)` - the CI runner's default iOS SDK is 26.2 while the deployment target is 15.6/18.2; harmless (objects still link). FIFTH run (29855648749): link GREEN, `** BUILD SUCCEEDED **`, BOTH app + appex codesigned with the correct named profiles - then Tauri's OWN `xcodebuild -exportArchive` failed: `error: exportArchive "Canari.app" requires a provisioning profile with the App Groups, Associated Domains, and Push Notifications features` (+ appex needs App Groups). ROOT CAUSE: the profiles are FINE (verified: `Canari` carries aps-environment/associated-domains=*/app-groups; `CanariNotifications` carries app-groups) - the archive signed correctly - but the ExportOptions.plist Tauri auto-generates can't express a TWO-target (app + appex) manual-signing profile map, so xcodebuild falls back to automatic signing on export and can't match. FIX (commit pending): split the build - `bun tauri ios build` now runs tolerantly (`|| warning`) to produce the signed `.xcarchive`, then a new "Export signed IPA" step writes our OWN ExportOptions.plist (`signingStyle=manual`, `provisioningProfiles` = {fr.emse.canari->Canari, fr.emse.canari.notifications->CanariNotifications}, teamID from APPLE_TEAM_ID, `Apple Distribution` cert, method=app-store-connect) and runs `xcodebuild -exportArchive` against the archive found under `gen/apple/build`. IPA lands at `gen/apple/build/ipa/*.ipa` (Locate step merged into the export step, id still `outputs`). NEXT: push + re-run + watch. If the manual export still errors, read the `xcdistributionlogs`/exportArchive error; possible next issues = archive path glob miss (the step prints the build tree on miss) or a codesign identity name mismatch. NOTE: WP-iOS-10 lproj strings are NOT yet referenced in the pbxproj (Léon didn't add the localized variant group) - permission prompts stay FR-only until wired. Guide: `docs/wiki/infrastructure/ios-ci-cd-setup.md`.
