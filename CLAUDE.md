...
* \[~\] **Carte de la Vie Asso - editable poster generator (design doc `docs/wiki/carte-vie-asso.md`; ALL browser-verify pending - snapdom/jsPDF can't run on host).** Data-driven re-editable poster; access `GlobalAdminOrBdeSuperAdminGuard`; project = opaque persisted JSON `layout` blob (content re-resolved from live data at render). Files: `lib/carte/{theme,generator,export,layout,shapes}.ts` + `components/carte/PosterCanvas.svelte` + `components/ui/ColorPicker.svelte` + `/admin/carte/[id]/+page.svelte`. GOTCHAS: prod applies `.sql` migrations manually; `format` already bakes `-c ../oxfmt.json` (don't re-pass); `{@const}` must be an immediate child of a block (put logo-dims consts at the `{#if data}` top, not inside the unit `<div>`). **Shipped (all on main):** Base P0-P3 + Refonte Steps 1-3 + feedback pass 1 `fix carto` (1b32a339: migration 021 no longer seeds categories - fully admin-managed; `isolation:isolate` on stage fixes send-to-back-behind-bg; A2 frame `STAGE_HEIGHT=round(W/SQRT2)=1131`, `DIRECTORY_WIDTH=500`; `fitSeedScale()` fits the whole roster in-frame = "missing assos" fix; directory `$effect` auto-shrinks font to fit) + feedback pass 2: WP1 `60f38a6` (shared `ColorPicker` with new optional `onChange` prop used for bubble/deco color; editable persisted `titleColor`; canvas zoom toolbar 0.4-4x over a scroll wrapper sized to the scaled poster; drag no longer clamps to margins/directory - reaches+overflows edges, KEEP=40px; full-page toggle moved into Reglages; **debounced autosave 4s** armed after hydration, reads only content fields) + WP2 `0835b343` (blob relayered: plain color silhouette back, hero logo its own frame shape centered + allowed to overflow, bureau member-cards over the top arc in front, **president always a card** photo+name+role at blob bottom, name below; bureau+president share a `memberCard` snippet with full name + role; **dedicated `LOGO_SHAPES`** round/squircle/wide/tall via `PositionedBubble.logoShape` + picker; directory shows roles in parens). Current geometry: `CARD_WIDTH=400 CARD_HEIGHT=430 BLOB_SIZE=200 BLOB_CY=168 LOGO_BASE=112 RING_RADIUS=124 CARD_W=78 PRES_CARD_W=104`, top-arc `-PI+((i+0.5)/n)*PI`.
  * WP3 `377ab61b` = **searchable A0 PDF export (user chose the HYBRID raster+text approach).** New shared `lib/pdf/searchableRaster.ts::exportSearchablePdf(el, opts)`: measures every `[data-pdf-text]` box from the live DOM (reproduces on-screen wrapping + directory auto-shrink for free), sets those runs transparent, rasterises the visuals via the shared snapdom helper as a full-bleed A0 background, then re-draws the text as REAL vector text (jsPDF, Helvetica) at the measured boxes -> crisp + selectable/Ctrl-F over a pixel-faithful background, signature unchanged (still takes the DOM node). `carte/export.ts` rewired to it (`format:'a0'` landscape). PosterCanvas text nodes tagged `data-pdf-text` (title / card name+role / asso name / directory heading+asso+members / free-text-only-when-non-empty). KNOWN TRADE-OFF: overlay text is Helvetica not Fredoka/Nunito (crisp, embed-free) - font embedding is an optional follow-up. FOLLOW-UPS: (a) apply the same helper to `calendarExport.ts` (agenda) once the carte export is verified good - user wants it factored/reused; (b) directory members `<p>` split across the 2 columns could misplace its overlay box (edge case). CANNOT be verified on this host (jsPDF/snapdom are browser-only) - user must check: export -> A0 landscape, text crisp + Ctrl-F works, visuals match preview.
