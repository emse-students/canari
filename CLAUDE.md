...
**C. MLS on a fresh device - notification decrypt + history bundle (DONE, committed, NOT pushed; on-device APK/TestFlight verify REQUIRED).** Two independent root causes, two commits:
* **B - history (ce9d99f1):** the Welcome-join path never solicited a history bundle (only external-join did), AND the background add path (`send-welcome-and-commit`) delivers Welcome+commit but no bundle -> a device added by a killed inviter got membership, no history. Fix: `frontend/src/lib/utils/chat/historySolicit.ts` (bounded, receipt-driven `solicitHistory`, cancelled on `history_bundle` receipt + on logout) wired into BOTH join paths (external join `recovery.ts`, Welcome join `sessionAuth.onWelcomeProcessed` new-conv only). Server `notifyHistoryRequest` now forwards to a RANDOM online member (Fisher-Yates) so client retries rotate past a frozen-online Android. Fully test-gated (vitest 5 + jest 4).
* **A - background decrypt (3aa3bf5a):** a device added to a group advances the epoch; a never-opened mobile only runs the read-only background decrypt (no commit apply, no persist) so it stays behind and the newcomer's 1st msg at the new epoch is an epoch gap -> generic fallback. Fix: read-only in-memory commit catch-up before fallback - `background_group_epoch` + `decrypt_push_message_with_commits` + `decode_commits_b64_json` (`mobile/background.rs`), `POST /api/mls/push/commits` (PushSecret twin of the JWT commits endpoint), Android JNI+Kotlin (`tryDecryptWithCommitCatchup` in the FCM decrypt-fail path), iOS FFI+Swift (`canari_push.mm`). NEVER persists mls.bin (foreground commit-log replay does the durable catch-up).
* **GOTCHA - verification gap:** the mobile Rust (JNI/FFI), Kotlin, and Swift are compile-UNVERIFIED here (no Android NDK clang, no Xcode); host `cargo check` skips `cfg(android/ios)` modules. Crypto path validated by `mls-core/tests/commit_catchup.rs` (2). `nativeProcessBackgroundTasks` is STILL a janitor (does NOT apply commits) - the memory [[project_notif_decrypt_readonly_limit]] core claim holds. Next: build an APK/TestFlight and verify end-to-end (fresh device -> backgrounded mobile decrypts + gets history), then push.
