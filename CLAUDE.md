...
* \[~\] **N7 CD via GitHub Secrets - CODE DONE + PUSHED (f135b0e).** deploy.yml now wipes `~/portail-etu` and rewrites `.env` from 4 GitHub Secrets each push (via `env:`, masked; Bun auto-loads `.env`; empty values fall back to public defaults / disable avatar proxy). Dropped dead sharp (rollup external + `bun install ... sharp` in deploy.yml/release.yml) + stale commented Setup-Bun block. `.env.example` is the canonical secret-name list. 3 URL secrets (`PUBLIC_CANARI_URL`/`GALLERY_API_URL`/`PORTAL_URL`) set via `gh secret set`. **REMAINING:** set `GALLERY_API_KEY` (rotate on MiGallery first, then `gh secret set GALLERY_API_KEY --repo emse-students/refonte-portail-etu`); it is the only true secret. Verify first post-setup deploy serves avatars.
