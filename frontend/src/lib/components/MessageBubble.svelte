...
<div class="flex w-full {isOwn ? 'justify-end' : 'justify-start'} group">
  <div class="flex flex-col gap-1 max-w-[75%] relative">
    ...
    >
      {#if replyTo}
        <div
          class="mb-2 pb-2 border-l-4 border-gray-400 pl-3 text-xs opacity-70"
        >
    ...
    {#if showEmojiPicker}
      <div
        class="absolute bottom-full mb-1 {isOwn
          ? 'right-0'
          : 'left-0'} bg-white border border-cn-border rounded-lg shadow-lg p-2 flex gap-1 z-10"
      >
        {#each quickEmojis as emoji (emoji)}
          <button
            onclick={() => handleEmojiClick(emoji)}
            class="p-2 hover:bg-gray-100 rounded-lg text-xl transition-colors"
          >
            {emoji}
          </button>
        {/each}
