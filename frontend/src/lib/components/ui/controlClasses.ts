...
/** Everything that does not depend on validity. */
const BASE =
  'text-text-main placeholder:text-text-muted/50 disabled:bg-cn-border/20 w-full rounded-2xl ' +
  'border-2 bg-(--cn-surface) px-4 py-3 text-base transition-all outline-none ' +
...
export function controlClass(invalid = false): string {
  return `${BASE} ${invalid ? EDGE_INVALID : EDGE_OK}`;
}
