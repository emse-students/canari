<script lang="ts">
  ...
  const Icon = $derived(
    preview?.kind === 'form' ? ClipboardList
    : preview?.kind === 'association' ? Building2
    : preview?.kind === 'profile' ? User
    : FileText
  );
</script>
...
>
  {#if loading}
    <div class="absolute inset-0 bg-black/15 dark:bg-white/10 animate-pulse" aria-hidden="true"></div>
  ...
  {:else}
    <img
      ...
      aria-hidden="true"
    />
    <Icon size={28} class="text-amber-300/95 relative z-[1]" strokeWidth={1.75} aria-hidden="true" />
  {/if}
</div>
