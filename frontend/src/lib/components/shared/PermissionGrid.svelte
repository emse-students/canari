...
{#if permissions.length === 0}
  <p class="text-sm text-text-muted italic">Aucune permission configurable.</p>
{:else}
  <div class="overflow-x-auto custom-scrollbar">
    <table class="w-full border-collapse text-xs" cellspacing="0">
      <thead>
        <tr>
          <th
            class="sticky left-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm text-left px-3 py-2.5 font-bold uppercase tracking-wider text-[0.65rem] text-text-muted border-b border-black/5 dark:border-white/10 min-w-[140px]"
          >
            Permission
          </th>
          {#each sortedRoles as role (role.id)}
            <th
              class="px-3 py-2.5 font-bold uppercase tracking-wider text-[0.65rem] text-text-muted border-b border-black/5 dark:border-white/10 text-center min-w-[80px]"
            >
              <span
