<script lang="ts">
  ...
  let { question = $bindable(), optionsRaw = $bindable(), multipleChoice = $bindable(), onRemove }: Props = $props();
</script>
...
<div class="rounded-2xl border border-cn-border/60 bg-cn-surface/70 dark:bg-black/25 p-5 shadow-sm ring-1 ring-black/[0.02] dark:ring-white/[0.04]">
  ...
  <div class="space-y-4">
    <Input label="Question du sondage" bind:value={question} placeholder="Quelle est votre question ?" />
    <Textarea label="Options (une par ligne)" bind:value={optionsRaw} rows={3} />

    <!-- Multiple-choice toggle -->
    <label class="flex cursor-pointer select-none items-center justify-between rounded-xl bg-cn-surface/80 px-4 py-3 transition-colors hover:bg-cn-border/30 dark:bg-white/5 dark:hover:bg-white/10">
