<script lang="ts">
  import { BarChart2, CheckCircle2, Circle, CheckSquare, Square } from 'lucide-svelte';
...
{#if polls && polls.length > 0}
  <div class="px-5 py-4 space-y-5">
    {#each polls as poll (poll.id)}
      ...
      <div class="rounded-[1.5rem] border border-black/5 dark:border-white/10 bg-white/60 dark:bg-black/20 backdrop-blur-xl p-5 shadow-sm">
        ...
        <div class="flex items-start gap-3 mb-5">
          ...
          <h4 class="font-extrabold text-[1.05rem] text-text-main leading-snug">
            ...
            {#if poll.multipleChoice}
              <span class="block text-[0.65rem] font-bold text-text-muted uppercase tracking-wider mt-1.5 opacity-80">
        ...
        <div class="space-y-2.5">
          {#each poll.options as option (option.id)}
            ...
            >
              ...
              {#if totalVotes > 0}
                <div
                  class="absolute inset-y-0 left-0 bg-black/5 dark:bg-white/5 transition-all duration-1000 ease-out z-0 {isSelected ? 'bg-amber-500/10 dark:bg-amber-500/15' : ''}"
              ...
              <div class="relative z-10 flex justify-between items-center gap-4">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div class="shrink-0 transition-colors {isSelected ? 'text-amber-500' : 'text-text-muted group-hover:text-amber-500'}">
                    {#if poll.multipleChoice}
                      {#if isSelected}
                        <CheckSquare size={20} strokeWidth={2.5} class="text-amber-500" />
                      {:else}
                        <Square size={20} strokeWidth={2} class="opacity-40 group-hover:opacity-100 transition-opacity" />
                      {/if}
                    {:else}
                      {#if isSelected}
                        <CheckCircle2 size={20} strokeWidth={2.5} class="text-amber-500" />
                      {:else}
                        <Circle size={20} strokeWidth={2} class="opacity-40 group-hover:opacity-100 transition-opacity" />
                      {/if}
                    {/if}
                  </div>
                ...
                <div class="flex items-center gap-2.5 shrink-0">
                  {#if totalVotes > 0}
                    <span class="text-xs font-extrabold text-text-main/60 min-w-[2.5rem] text-right">
                      {percentage}%
                    </span>
                  {/if}
                  <span
