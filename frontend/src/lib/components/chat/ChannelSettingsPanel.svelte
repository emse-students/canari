<script lang="ts">
  ...
  interface Props {
     ...
     * Dismisses the panel.
     *
     * NOT the close button - that lives in `ConversationSidePanel`'s header with every other
     * panel's. This is for the two actions that destroy the thing being configured: after leaving
  ...
  /*
   * (Re)load the notification level whenever the panel lands on a different channel.
   *
   * THE `open &&` GUARDS THAT WERE HERE WERE SILENTLY TRUE. `open` stopped being a prop when this
   * became panel content, and the identifier still resolved - to `window.open`, a function, which
   * is truthy. TypeScript caught it ("this condition will always return true since this function is
   * always defined"); nothing at runtime would have. The state they guarded is now the difference
   * between mounted and not.
   */
  $effect(() => {
    if (selectedChannelId && notifLoadedFor !== selectedChannelId) {
...
<!--
  CONTENT ONLY, AND THE STACKED LAYOUT IS NOW THE ONLY ONE. This was a `max-w-4xl` modal with a
  16rem tab rail beside its content from `md` up - a fourth way to show something about a
  conversation, over the conversation, in a shape nothing else in the app uses. It renders into
  `ConversationSidePanel` now, where there is one column, so the tab strip is the horizontal
  scroller this file already had for phones. The wide two-column form is deleted rather than kept
