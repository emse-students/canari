...
<aside
  class="h-full backdrop-blur-md bg-white/40 dark:bg-gray-900/50 border-r border-white/50 dark:border-white/10 flex {viewMode === 'communities' ? 'flex-row' : 'flex-col'} {drawerMode
    ? 'fixed left-0 top-0 bottom-0 z-40 md:hidden shadow-2xl animate-panel-in ' + (viewMode === 'communities' ? 'w-[95vw] max-w-md' : 'w-[88vw] max-w-sm')
    : (viewMode === 'communities' ? 'w-full md:w-96' : 'w-full md:w-80')} {isHidden && !drawerMode ? 'hidden md:flex' : ''}"
>
  {#if viewMode === 'communities'}
    <div class="w-[72px] h-full flex-shrink-0 flex flex-col items-center py-3 gap-3 border-r border-white/50 dark:border-white/10 bg-white/20 dark:bg-black/10 overflow-y-auto no-scrollbar">
      {#each channelWorkspaces as workspace (workspace.id)}
        <button
          class="relative w-12 h-12 flex-shrink-0 rounded-2xl overflow-hidden transition-all duration-200 {selectedCommunityWorkspaceId === workspace.id ? 'ring-2 ring-cn-yellow ring-offset-2 ring-offset-cn-bg' : 'opacity-70 hover:opacity-100 hover:rounded-xl'}"
      ...
      <button
        onclick={() => { showNewCommunityModal = true; }}
  ...
  <div class="flex-1 h-full min-w-0 flex flex-col">
    {#if viewMode === 'chat'}
      ...
      />
    {:else}
      <div class="px-4 py-3 border-b border-white/50 dark:border-white/10 bg-white/30 dark:bg-gray-900/40 backdrop-blur-sm sticky top-0 z-10 flex items-center justify-between">
        <h2 class="font-black tracking-tight text-text-main text-lg truncate">
          {channelWorkspaces.find(w => w.id === selectedCommunityWorkspaceId)?.name || 'Communautés'}
        </h2>
        ...
        <div class="flex items-center gap-1">
          <button
            ...
            onclick={() => { showCommunityAdminModal = true; }}
    ...
    {#if activeSidebarTab === 'discussions'}
      ...
      {/if}
    {:else}
      {@const currentWorkspace = channelWorkspaces.find(w => w.id === selectedCommunityWorkspaceId)}
      {#if currentWorkspace}
        <div class="px-2 py-2">
      ...
      {:else}
        <div class="text-center py-8 px-4 text-text-muted text-sm">Sélectionner ou créer une communauté.</div>
      {/if}
    {/if}
  </div>
