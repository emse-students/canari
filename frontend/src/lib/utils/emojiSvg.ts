 ...
 *    TEXT. A grapheme is a picture only when Unicode says it presents as an emoji: it carries U+FE0F,
 *    or one of its code points is `Emoji_Presentation` (which covers flags, skin tones and the
 *    pictographs that are emoji by default). This is the `RGI_Emoji` rule without the regex `v` flag,
 *    which the WebKit of iOS 16 cannot parse - a SyntaxError at module load would blank the app.
...
/** Whether Unicode presents this grapheme as an emoji rather than as text (gate 1 above). */
export function presentsAsEmoji(grapheme: string): boolean {
  for (const char of grapheme) {
    if (char.codePointAt(0) === VS16) return true;
  }
