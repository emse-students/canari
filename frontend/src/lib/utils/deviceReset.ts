 ...
 *
 * Its caller is a device the server has revoked (`wipeRevokedDevice`), and that is why it is more
 * than an MLS wipe - a revoked device is one its owner has declared lost or stolen, so leaving its
 * cached avatars, its drafts, its theme and its conversation list behind answers the wrong question. What must remain afterwards is
...
export async function wipeDeviceToFactory(): Promise<string[]> {
  ...
  // THE SURVEY IS SYNCHRONOUS ON PURPOSE, and a second, later look was deliberately NOT added: a
  // user who signs back in writes keys within seconds, so a delayed audit would accuse an ordinary new session of being a zombie. What
