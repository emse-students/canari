/**
 * Contracts for preventive layer: commit baseEpoch must match chat-delivery
 * `POST /api/mls/commit` (epoch-gated validation). See ARCHITECTURE.md in this package.
 */
...
describe('desync prevention: commitBaseEpochForValidation', () => {
  it('matches POST /api/mls/commit expectation (local epoch N → baseEpoch N-1)', () => {
