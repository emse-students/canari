...
/**
 * True at most once per group per {@link SIGNAL_INTERVAL_MS}.
 *
 * A rewound sender does not fail one frame, it fails every frame it sends until its ratchet passes
 * the generations we consumed. Signalling each of them would answer a storm with a storm, and each
 * signal asks the peer to retransmit.
 */
export function shouldSignalDesync(groupId: string, now: number = Date.now()): boolean {
  ...
  if (last !== undefined && now - last < SIGNAL_INTERVAL_MS) return false;
